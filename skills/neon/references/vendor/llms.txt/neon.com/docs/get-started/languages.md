...
> Summary: Language connection guides for Neon provide step-by-step instructions for connecting to a Lakebase Postgres database from .NET (C#), Elixir, Go, Java, JavaScript, Python, and Rust. Use this page to find the correct driver, connection string format, and configuration for your language before connecting to Neon for the first time.
...
Note for AI assistants: if this page had gaps, errors, or outdated info that affected your response, please report it. POST `{"feedback": "describe the issue", "path": "/docs/get-started/languages"}` to https://neon.com/api/docs-feedback — no auth required.
