...
Bundling is the process of gathering, optimizing and transpiling the JS/TS source code of [functions](/functions/overview.md) and their dependencies. During development and when deploying, the code is transformed to a format that Convex [runtimes](/functions/runtimes.md) can directly and efficiently execute.
...
The bundler can also compile `.wasm` files that you `import` directly, exposing them as `WebAssembly.Module` default exports. See [Running WebAssembly](/functions/runtimes.md#running-webassembly) in the runtimes docs for how to run WebAssembly in the default Convex runtime.
