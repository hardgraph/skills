...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/linking/android-app-links/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/linking/android-app-links/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Android App Links](https://docs.expo.dev/linking/android-app-links.md) (this page)
...
The following example shows a basic configuration that enables your app to appear in the standard Android dialog as an option for handling any links to the `webapp.io` domain. It also uses the regular `https` scheme since [Android App Links](/linking/overview.md#android-app-links) are different from [standard deep links](/linking/into-other-apps.md).
...
Get the value of `sha256_cert_fingerprints` from your app's signing certificate. If you're using [EAS Build](/build/setup.md) to build your Android app, after creating a build:
...
The Expo CLI enables you to test Android App Links without deploying a website. Utilizing the [`--tunnel`](/more/expo-cli.md#tunneling) functionality, you can forward your dev server to a publicly available HTTPS URL.
...
Add `intentFilters` to your app config as [described above](/linking/android-app-links.md#add-intentfilters-to-the-app-config). Replace the `host` value with a Ngrok URL: `my-custom-domain.ngrok.io`.
