...
Credentials for integrations supported by n8n, including both built-in and community nodes. Use **Predefined Credential Type** for custom operations without extra setup. Refer to [Custom API operations](/integrations/builtin/custom-api-actions-for-existing-nodes.md) for more information.
...
For example setups, refer to [HTTP Request node cookbook | Pagination](/build/code-in-n8n/cookbook/http-request-node/pagination.md).
...
Enter the **Proxy** the request should use. This takes precedence over global settings defined with the [`HTTP_PROXY`, `HTTPS_PROXY`, or `ALL_PROXY` environment variables](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables/deployment.md).
...
The following options are only available when attached to an [AI agent](/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent/tools-agent.md) as a [tool](/build/integrate-ai/understand-ai-components/how-tools-work.md){ data-preview }.
...
Set this up in the [Authentication](#authentication) section. Use a **Predefined Credential Type** when n8n supports the service, since it's the easiest to set up. For other APIs, use generic credentials, which support Basic auth, Header auth, OAuth1, OAuth2, and more. Refer to [HTTP Request credentials](/integrations/builtin/credentials/httprequest.md).
