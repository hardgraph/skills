...
Human in the Loop works with any [Browser Session](https://developers.cloudflare.com/browser-run/#integration-methods) and provides two approaches for human intervention:
...
2. The script retrieves the [Live View](https://developers.cloudflare.com/browser-run/features/live-view/) URL from the session's target list and shares it with a human operator.
...
Generates a [Live View](https://developers.cloudflare.com/browser-run/features/live-view/) URL for the browser session. Use this alongside `Cloudflare.handoff` to give the human operator access to the live browser.
