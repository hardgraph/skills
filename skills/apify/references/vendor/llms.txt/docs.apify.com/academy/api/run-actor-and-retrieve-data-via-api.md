...
url: https://docs.apify.com/academy/api/run-actor-and-retrieve-data-via-api.md
parents:
  - [Apify documentation](https://docs.apify.com/llms.txt)
  - [Apify Academy](https://docs.apify.com/academy.md)
  - [Tutorials on Apify API](https://docs.apify.com/academy/api.md)
previous: [How to retry failed requests](https://docs.apify.com/academy/api/retry-failed-requests.md)
next: [Use Apify via API from PHP](https://docs.apify.com/academy/php/use-apify-from-php.md)
---
...
> If you are unsure about the differences between an Actor and a task, you can read about them in the [tasks](https://docs.apify.com/actors/running/tasks.md) documentation. In brief, tasks are pre-configured inputs for Actors.

The API endpoints and usage (for both sync and async) for [Actors](https://docs.apify.com/api/v2/actors-runs-post.md) and [tasks](https://docs.apify.com/api/v2/actor-task-runs-post.md) are essentially the same.
...
* Your [API token](https://docs.apify.com/integrations.md), which you can find on the **API & Integrations** page in [Apify Console](https://console.apify.com/settings/integrations) (do not share it with anyone!).
...
This works in almost exactly the same way for both Actors and tasks; however, for tasks, there is no reason to specify a [build](https://docs.apify.com/actors/development/builds-and-runs/builds.md) parameter, as a task already has only one specific Actor build which cannot be changed with query parameters.
...
We will later use this **run info** JSON to retrieve the run's output data. This info about the run can also be retrieved with another call to the [Get run](https://docs.apify.com/api/v2/actor-run-get.md) endpoint.
...
You can also use the `waitForFinish` parameter with the [GET Run endpoint](https://docs.apify.com/api/v2/actor-run-get.md) to implement a smarter polling system.
...
If you have a server, [webhooks](https://docs.apify.com/integrations/webhooks.md) are the most elegant and flexible solution for integrations with Apify. You can set up a webhook for any Actor or task, and that webhook will send a POST request to your server after an [event](https://docs.apify.com/integrations/webhooks/events.md) has occurred.
...
Once a status of `SUCCEEDED` or `FAILED` has been received, we know the run has finished and can cancel the interval and finally collect the data.

### Collecting the data

Unless you used the synchronous call mentioned above, you will have to make one additional request to the API to retrieve the data.

The **run info** JSON also contains the IDs of the default [dataset](https://docs.apify.com/storage/dataset.md) and [key-value store](https://docs.apify.com/storage/key-value-store.md) that are allocated separately for each run, which is usually everything you need. The fields are called `defaultDatasetId` and `defaultKeyValueStoreId`.
...
> [Key-value stores](https://docs.apify.com/storage/key-value-store.md) are mainly useful if you have a single output or any kind of files that cannot be [stringified](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify) (such as images or PDFs).
