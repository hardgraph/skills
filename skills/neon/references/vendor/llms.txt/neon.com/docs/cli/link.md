...
| `--branch`, `--branch-id` | Branch name or ID to pin in the context (resolved to its ID before writing). Without it, link only resolves the org and project — pin a branch with `neon checkout <branch>` (link never guesses a default).                                                  | string  | —       |    No    |
