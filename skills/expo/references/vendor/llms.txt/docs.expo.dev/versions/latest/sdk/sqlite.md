...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/sdk/sqlite/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/sdk/sqlite/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
The [`with-sqlite`](https://github.com/expo/examples/tree/master/with-sqlite) example comes with `expo-sqlite` already installed and configured:
...
> Web support is in [alpha](/more/release-statuses.md#alpha) and may be unstable. [Create an issue on GitHub](https://github.com/expo/expo/issues) if you encounter any issues.
...
Add the following configuration to your **metro.config.js**. If you don't have the **metro.config.js** yet, you can run `npx expo customize metro.config.js`. [Learn more about customizing Metro](/guides/customizing-metro.md).
...
If you deploy your app on [EAS Hosting](/eas/hosting/introduction.md), you can configure the headers in your app config:
...
Prepared statements allow you to compile your SQL query once and execute it multiple times with different parameters. They automatically escape input parameters to defend against SQL injection attacks, and are recommended for queries that include user input. You can get a prepared statement by calling [`prepareAsync()`](/versions/latest/sdk/sqlite.md#prepareasyncsource) or [`prepareSync()`](/versions/latest/sdk/sqlite.md#preparesyncsource) method on a database instance. The prepared statement can fulfill CRUD operations by calling [`executeAsync()`](/versions/latest/sdk/sqlite.md#executeasyncparams) or [`executeSync()`](/versions/latest/sdk/sqlite.md#executesyncparams) method.

> **Note:** Remember to call [`finalizeAsync()`](/versions/latest/sdk/sqlite.md#finalizeasync) or [`finalizeSync()`](/versions/latest/sdk/sqlite.md#finalizesync) method to release the prepared statement after you finish using the statement. `try-finally` block is recommended to ensure the prepared statement is finalized.
...
As with the [`useSQLiteContext()`](/versions/latest/sdk/sqlite.md#usesqlitecontext-hook) hook, you can also integrate the [`SQLiteProvider`](/versions/latest/sdk/sqlite.md#sqlitesqliteprovider) with [`React.Suspense`](https://react.dev/reference/react/Suspense) to show a fallback component until the database is ready. To enable the integration, pass the `useSuspense` prop to the `SQLiteProvider` component.
...
The [`withExclusiveTransactionAsync()`](/versions/latest/sdk/sqlite.md#withexclusivetransactionasynctask) function addresses this. Only queries that run within the scope function passed to `withExclusiveTransactionAsync()` will run within the actual SQL transaction.
...
To open a new SQLite database using an existing **.db** file you already have, you can use the [`SQLiteProvider`](/versions/latest/sdk/sqlite.md#sqlitesqliteprovider) with [`assetSource`](/versions/latest/sdk/sqlite.md#assetsource).
...
Use [`Paths.appleSharedContainers`](/versions/latest/sdk/filesystem.md#applesharedcontainers) from the [`expo-file-system`](/versions/latest/sdk/filesystem.md) library to retrieve the path to the shared container:
...
SQL injections are a class of vulnerabilities where attackers trick your app into executing user input as SQL code. You must escape all user input passed to SQLite to defend against SQL injections. [Prepared statements](/versions/latest/sdk/sqlite.md#prepared-statements) are an effective defense against this problem. They explicitly separate a SQL query's logic from its input parameters, and SQLite automatically escapes inputs when executing prepared statements.
...
[SQLCipher](https://www.zetetic.net/sqlcipher/) is a fork of SQLite that adds encryption and authentication to the database. The `expo-sqlite` library supports SQLCipher for Android, iOS, and macOS. To use SQLCipher, you need to add the `useSQLCipher` config to your **app.json** as shown in the [Configuration in app config](/versions/latest/sdk/sqlite.md#configuration-in-app-config) section and run `npx expo prebuild`.
...
The following table summarizes the common API for [`SQLiteDatabase`](/versions/latest/sdk/sqlite.md#sqlitedatabase) and [`SQLiteStatement`](/versions/latest/sdk/sqlite.md#sqlitestatement) classes:

| [`SQLiteDatabase`](/versions/latest/sdk/sqlite.md#sqlitedatabase) methods | [`SQLiteStatement`](/versions/latest/sdk/sqlite.md#sqlitestatement) methods | Description | Use case |
| --- | --- | --- | --- |
| [`runAsync()`](/versions/latest/sdk/sqlite.md#runasyncsource-params) | [`executeAsync()`](/versions/latest/sdk/sqlite.md#executeasyncparams) | Executes a SQL query, returning information on the changes made. | Ideal for SQL write operations such as `INSERT`, `UPDATE`, `DELETE`. |
| [`getFirstAsync()`](/versions/latest/sdk/sqlite.md#getfirstasyncsource-params) | [`executeAsync()`](/versions/latest/sdk/sqlite.md#executeasyncparams) + [`getFirstAsync()`](/versions/latest/sdk/sqlite.md#getfirstasync) | Retrieves the first row from the query result. | Suitable for fetching a single row from the database. For example: `getFirstAsync('SELECT * FROM Users WHERE id = ?', userId)`. |
| [`getAllAsync()`](/versions/latest/sdk/sqlite.md#getallasyncsource-params) | [`executeAsync()`](/versions/latest/sdk/sqlite.md#executeasyncparams) + [`getFirstAsync()`](/versions/latest/sdk/sqlite.md#getallasync) | Fetches all query results at once. | Best suited for scenarios with smaller result sets, such as queries with a LIMIT clause, like `SELECT * FROM Table LIMIT 100`, where you intend to retrieve all results in a single batch. |
| [`getEachAsync()`](/versions/latest/sdk/sqlite.md#geteachasyncsource-params) | [`executeAsync()`](/versions/latest/sdk/sqlite.md#executeasyncparams) + `for-await-of` async iterator | Provides an iterator for result set traversal. This method fetches one row at a time from the database, potentially reducing memory usage compared to `getAllAsync()`. | Recommended for handling large result sets incrementally, such as with infinite scrolling implementations. |
...
Type: [MemoExoticComponent](https://react.dev/reference/react/memo)<(__namedParameters: [SQLiteProviderProps](/versions/latest/sdk/sqlite.md#sqlitesqliteprovider)) => [Element](https://www.typescriptlang.org/docs/handbook/jsx.html#function-component)\>
...
A global hook for accessing the SQLite database across components. This hook should only be used within a [`<SQLiteProvider>`](#sqlitesqliteprovider) component.
...
| --- | --- |
| `key` | `string` |
| `value` | string | [SQLiteStorageSetItemUpdateFunction](/versions/latest/sdk/sqlite.md#sqlitestoragesetitemupdatefunctionprevvalue) |
