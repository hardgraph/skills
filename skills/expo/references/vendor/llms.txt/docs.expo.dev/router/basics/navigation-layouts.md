...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/router/basics/navigation-layouts/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/router/basics/navigation-layouts/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Layout](https://docs.expo.dev/router/basics/navigation-layouts.md) (this page)
...
On Android and iOS, you can use [native tabs](/router/advanced/native-tabs.md) to render the platform's built-in tab bar. Native tabs provide expected platform behaviors like scroll-to-top on tap, native animations, and a native look and feel.
...
Since native tabs are only available on Android and iOS, a common pattern is to use [platform-specific file extensions](/router/advanced/platform-specific-modules.md) to provide different tab implementations for native and web. The root layout renders a tab component, and Expo's module resolution automatically picks the correct file based on the platform.
...
On Android and iOS, **app-tabs.native.tsx** uses [native tabs](/router/advanced/native-tabs.md):
...
On web, **app-tabs.tsx** uses [custom tabs](/router/advanced/custom-tabs.md) from `expo-router/ui`, which are unstyled and flexible components:
...
-   Implement a [Drawer navigator](/router/advanced/drawer.md)
-   Use [native tabs](/router/advanced/native-tabs.md) for a platform-native tab bar on Android and iOS
-   Replace the default tabs with [fully customized tabs](/router/advanced/custom-tabs.md)
-   Use a [modal](/router/advanced/modals.md) to display a page with transparency, such that the parent navigator is still visible underneath
-   [Adapt any navigator that is compatible with React Navigation](/versions/latest/sdk/router.md#withlayoutcontextnav-processor-useonlyuserdefinedscreens), including top tabs, bottom sheets, and more
