...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/sdk/audio/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/sdk/audio/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
To enable background audio playback, use the config plugin in your [app config](/workflow/configuration.md):
...
> **Note**: On Android, you have to enable the lock screen controls with [`setActiveForLockScreen`](/versions/latest/sdk/audio.md#setactiveforlockscreenactive-metadata-options) for sustained background playback. Otherwise, the audio will stop after approximately 3 minutes of background playback (OS limitation). Ensure to appropriately [configure the config plugin](/versions/latest/sdk/audio.md#configuration-in-app-config).
...
If you're not using Continuous Native Generation ([CNG](/workflow/continuous-native-generation.md)) (you're using native **android** and **ios** projects manually), then you need to configure the following for background playback:
...
To enable background recording, use the config plugin in your [app config](/workflow/configuration.md):
...
If you're not using Continuous Native Generation ([CNG](/workflow/continuous-native-generation.md)) (you're using native **android** and **ios** projects manually), then you need to configure the following permissions in your native projects:
...
After configuring your app, enable background recording at runtime using [`setAudioModeAsync`](/versions/latest/sdk/audio.md#audiosetaudiomodeasyncmode):
...
In most cases, use the [`useAudioPlayer`](/versions/latest/sdk/audio.md#useaudioplayersource-options) hook to create an `AudioPlayer` instance. It manages the player's lifecycle and ensures proper disposal when the component unmounts. However, in some advanced use cases, you may need to create an `AudioPlayer` that persists beyond the component's lifecycle. In those cases, use the [`createAudioPlayer`](/versions/latest/sdk/audio.md#audiocreateaudioplayersource-options) function. You need to be aware of the risks that come with this approach, as it is your responsibility to call the [`release()`](/versions/latest/sdk/expo.md#release) method when the player is no longer needed. If not handled properly, this approach may lead to memory leaks.
...
| Parameter | Type |
| --- | --- |
| `options`(optional) | [AudioStreamOptions](#audiostreamoptions) |

  

Hook that creates a native audio stream for real-time PCM microphone capture. Call `stream.start()` to begin and `stream.stop()` to end. Requires microphone permission via `requestRecordingPermissionsAsync()`.

Returns: `AudioStreamResult`
...
Type: Class extends [SharedObject](/versions/v57.0.0/sdk/expo.md#sharedobjecttype)<[AudioEvents](#audioevents)\>
...
Type: Class extends [SharedObject](/versions/v57.0.0/sdk/expo.md#sharedobjecttype)<[AudioPlaylistEvents](#audioplaylistevents)\>
...
Type: Class extends [SharedObject](/versions/v57.0.0/sdk/expo.md#sharedobjecttype)<[RecordingEvents](#recordingevents)\>
...
Type: Class extends [SharedObject](/versions/v57.0.0/sdk/expo.md#sharedobjecttype)<[AudioStreamEvents](#audiostreamevents)\>
...
Begins capturing audio from the microphone. Requires microphone permission — call `requestRecordingPermissionsAsync()` first.
...
| Property | Type | Description |
| --- | --- | --- |
| channels | `number` | Actual number of channels. |
| data | [ArrayBuffer](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer) | Raw PCM audio data.
...
| sampleRate | `number` | Actual sample rate in Hz. May differ from the requested rate. |
| timestamp | `number` | Seconds since the stream was started. |
...
Acceptable values are: `'float32'` | `'int16'`
...
| Property | Type | Description |
| --- | --- | --- |
| audioStreamBuffer | (buffer: [AudioStreamBuffer](#audiostreambuffer)) => void | - |
| audioStreamStatus | (status: [AudioStreamStatus](#audiostreamstatus)) => void | - |
...
| Property | Type | Description |
| --- | --- | --- |
| channels(optional) | `number` | Number of audio channels. 1 for mono, 2 for stereo. Default: `1` |
| encoding(optional) | [AudioStreamEncoding](#audiostreamencoding) | PCM encoding format. Default: `'float32'` |
| onBuffer(optional) | (buffer: [AudioStreamBuffer](#audiostreambuffer)) => void | Called each time a new PCM buffer is captured from the microphone. |
| sampleRate(optional) | `number` | Desired sample rate in Hz. The actual rate may differ if the hardware cannot deliver it. Default: `48000` |
...
Acceptable values are: `'cache'` | `'document'`
...
| directory(optional) | [RecordingDirectory](#recordingdirectory) | Supported platforms: Android, iOS. The directory where the recording file should be stored. Default: `'cache'` |
