...
- Each `ThreadListItemPrimitive.Trigger` is a native `<button>` and its own Tab stop, so the whole list is reachable with `Tab` alone. The arrow keys (see [Keyboard Navigation](#keyboard-navigation)) speed traversal up, but nothing depends on them.
...
- `asChild`: `boolean` (default `false`) — Change the default rendered element for the one passed as a child, merging their props and behavior. [Composition guide](/docs/api-reference/primitives/composition)
