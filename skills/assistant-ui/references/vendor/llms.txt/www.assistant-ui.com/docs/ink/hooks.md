...
> [!info]
>
> An Ink app runs in a single Node process, so there is no client/server boundary to split and no build step. `defineToolkit` from `@assistant-ui/react-ink` runs at runtime: import it as a value, with no `"use generative"` directive and no `"use client"` inside `execute`.
