> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/deploy/host-n8n/configure-n8n/security/redact-execution-data.md).
...
2. Select the **three dots icon** <img src="/files/cswZmvezHpwEHCDZa9sT" alt="three dots icon" data-size="line"> in the upper-right corner.
3. Select **Settings**.
...
To enable enforcement, navigate to **Settings** > **Security** and configure the **Data redaction** section. Refer to [Security settings](/deploy/host-n8n/configure-n8n/security/manage-security-policies.md#enforce-execution-data-redaction) for the step-by-step instructions.
...
[Log streaming](/administer/observe-and-log/stream-logs-to-external-systems.md) tracks reveal actions and enforcement policy changes. The following audit events are available:
...
Execution data redaction introduces the following permission scopes that you can assign through [custom project roles](/administer/manage-users-and-access/set-permissions-and-roles-rbac/create-custom-roles.md):
...
* **Enforcement doesn't propagate through source control**: instance-level enforcement is an instance policy and isn't included when you push workflows with [source control](/administer/use-source-control-and-environments.md). A workflow pushed from an enforced instance to a non-enforced instance isn't redacted on the target instance. This matches the behavior of other instance-level policies, such as two-factor authentication enforcement.
...
* **Use log streaming**: Enable [log streaming](/administer/observe-and-log/stream-logs-to-external-systems.md) to capture reveal audit events. This provides an audit trail for compliance and allows you to monitor who accesses sensitive execution data.
...
* When you [create custom nodes](/connect/create-nodes/overview.md), you can declare specific output fields as sensitive (using `sensitiveOutputFields` in the node type definition). n8n always redacts these fields and prevents revealing them, even for users with reveal access.
...
* When redaction is enabled, execution data is also automatically redacted from [log streaming](/administer/observe-and-log/stream-logs-to-external-systems.md) and logging output.
