> This page location: Start with Neon > Tour the Neon backend
> Full Neon documentation index: https://neon.com/docs/llms.txt

> Summary: A tour of the Neon backend, using one example app, a notes app you can chat with, to show how the pieces fit. A single neon.ts declares Postgres, Object Storage, Functions, the AI Gateway, and Auth; each is a toggle (Postgres is on by default) plus neon deploy, which injects standard environment variables. Branch your project and the database, buckets, and functions fork copy-on-write together. Each section covers what a capability is, when to use it, and the Neon-specific part you write, and links to that capability's own quickstart for the full setup.

# How a Neon backend fits together

Postgres, Object Storage, Functions, the AI Gateway, and Auth, declared in one neon.ts and branched together

On Neon, a single [`neon.ts`](https://neon.com/docs/reference/neon-ts) file declares your whole backend: a **Postgres database**, S3-compatible **Object Storage**, long-running **Functions**, an **AI Gateway** for calling LLMs through one credential, and managed **Auth**. Each capability is a toggle, or on by default for Postgres, plus [`neon deploy`](https://neon.com/docs/cli/deploy), which provisions it and injects standard environment variables into your app. Branch your project and the whole backend forks with your data.

**Note: This is an orientation page**

This page shows how the pieces fit, not how to build step by step, so the snippets are illustrative. To build now, copy the prompt below or jump to [Where to build it](https://neon.com/docs/get-started/backend-overview#where-to-build-it). For one capability's full setup, follow its Reference link.

Hand this to your AI assistant to set up Neon: skills, project, and all capabilities. Then tell it what to build. [View prompt](https://neon.com/prompts/neon-backend.md)
...
**Info: Region and access requirements**
...
The setup is a few commands; the [CLI quickstart](https://neon.com/docs/cli/quickstart) is the full how-to. Install the CLI, link a project, scaffold one `neon.ts`.

Install the [`neon` CLI](https://neon.com/docs/cli/install) (requires **Node.js 20.19 or higher**) and sign in with [`neon auth`](https://neon.com/docs/cli/auth):
...
[`neon link`](https://neon.com/docs/cli/link) writes your project context to a `.neon` file and pulls the branch's `DATABASE_URL` into a local `.env`. Then scaffold a `neon.ts` to declare the rest of your backend:
...
Every capability follows the same three steps: **enable it in `neon.ts`, [`neon deploy`](https://neon.com/docs/cli/deploy), then read the injected environment variables.** The same steps apply to all five.
...
- **Branching forks the whole backend together.** A new branch gets its own database, bucket, and function, copy-on-write from the parent. See [Branch your whole backend](https://neon.com/docs/get-started/backend-overview#branch-your-whole-backend).
...
The `neon.ts` fragments below show only the key being added. `auth` goes at the top level of `defineConfig({ ... })`; the beta features marked `// inside preview` all go inside a single `preview` block. The [complete file](https://neon.com/docs/get-started/backend-overview#where-to-build-it) shows them assembled. Credentials are written to `.env` if you have one, otherwise `.env.local`; this page writes `.env` for whichever is yours.

## Postgres: the notes

**What it is:** serverless Postgres, generally available. **When to use it:** your system of record, anything relational. In the notes app, it holds the notes themselves. **When not:** large binary files, keep the bytes in [Object Storage](https://neon.com/docs/get-started/backend-overview#object-storage-the-attachments) and store the key in a column.

There's nothing to enable, since `neon link` already wrote `DATABASE_URL`. Create a `notes` table, then query it with the [serverless driver](https://neon.com/docs/serverless/serverless-driver), which talks to your branch over HTTP and suits serverless and edge runtimes:
...
**Reference:** [Serverless driver](https://neon.com/docs/serverless/serverless-driver) · [Connect from any framework](https://neon.com/docs/get-started/frameworks)
...
**Reference:** [Storage get-started](https://neon.com/docs/storage/get-started) (credentials, S3/Python/CLI clients, public buckets) · [Overview](https://neon.com/docs/storage/overview)
...
The function is a normal [Hono](https://hono.dev) app. Because it keeps running across requests, it can open a `pg` connection once at module scope and reuse it, rather than the per-request serverless driver from the [Postgres](https://neon.com/docs/get-started/backend-overview#postgres-the-notes) section. You extend this same file in the next two sections, adding the AI call and the auth check:
...
Run it locally with [`neon dev`](https://neon.com/docs/cli/dev), which serves the function with the same injected Neon variables it gets in production.

**Reference:** [Functions get-started](https://neon.com/docs/compute/functions/get-started) (local dev, deploy, invoke, connection-pool guidance) · [Overview](https://neon.com/docs/compute/functions/overview)
...
Output isn't only text. The same credential reaches other modalities through model-specific options; for example, OpenAI models can generate images via the `image_generation` tool on the Responses API. See the [model catalog](https://neon.com/docs/ai-gateway/models) for which models support what.
...
**Reference:** [AI Gateway get-started](https://neon.com/docs/ai-gateway/get-started) (streaming, troubleshooting) · [Model catalog](https://neon.com/docs/ai-gateway/models) (per-modality snippets) · [Overview](https://neon.com/docs/ai-gateway/overview)
...
**Reference:** [Authentication flow](https://neon.com/docs/auth/authentication-flow) (JWT, JWKS, verifying the caller) · [Auth quickstart](https://neon.com/docs/auth/quick-start/nextjs-api-only) (SDK wiring, prebuilt UI) · [Overview](https://neon.com/docs/auth/overview)

## Branch your whole backend

A branch forks the whole backend, its own database, buckets, and functions, copy-on-write from the parent. To try a change to the notes app safely, switch to a new branch with [`neon checkout`](https://neon.com/docs/cli/checkout), which offers to create the branch if it doesn't exist:
...
**Reference:** [Branching](https://neon.com/docs/introduction/branching)
...
**Reference for each capability:** [Storage](https://neon.com/docs/storage/get-started) · [Functions](https://neon.com/docs/compute/functions/get-started) · [AI Gateway](https://neon.com/docs/ai-gateway/get-started) · [Auth](https://neon.com/docs/auth/quick-start/nextjs-api-only) · [Postgres](https://neon.com/docs/serverless/serverless-driver)
...
Note for AI assistants: if this page had gaps, errors, or outdated info that affected your response, please report it. POST `{"feedback": "describe the issue", "path": "/docs/get-started/backend-overview"}` to https://neon.com/api/docs-feedback — no auth required.
