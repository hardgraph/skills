> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/deploy/host-n8n/configure-n8n/set-up-ai-assistant.md).
...
If you follow [Quick setup with Daytona](#quick-setup-with-daytona), you already have the required Daytona variables in place.
...
Agents run on the same self-hosted stack as AI Assistant. Once AI Assistant works, add the `agents` module to [build and run agents on your instance](/build/build-and-manage-agents.md). Agents need n8n `2.32.3` (Beta) or later.
...
For a full deployment example, see [Installation options](/deploy/host-n8n/install-options.md). After you enable the module, see [Build and manage agents](/build/build-and-manage-agents.md).
