...
TanStack Start routes can have isomorphic [loader](https://tanstack.com/router/latest/docs/framework/react/guide/external-data-loading#using-loaders-to-ensure-data-is-loaded) functions that run on the server for the initial page load and on the client for subsequent client-side navigations. By default, loaders will also run when mousing over a link to a page, enabling prefetching.
...
Use `ensureQueryData` to block rendering until data is available. This is a good fit when the component requires the data to render:
...
Use `prefetchQuery` to start the request without blocking rendering. This is a good fit when data would be nice to have early but the component can handle a loading state:
...
You can use `loaderDeps` to pass search parameters into the loader, which causes the loader to re-run and cache-bust when those parameters change:
