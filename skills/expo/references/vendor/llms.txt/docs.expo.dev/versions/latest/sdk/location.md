...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/sdk/location/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/sdk/location/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
If you're not using Continuous Native Generation ([CNG](/workflow/continuous-native-generation.md)) or you're using native **ios** project manually, then you need to add the `NSLocationAlwaysAndWhenInUseUsageDescription`, `NSLocationAlwaysUsageDescription` and `NSLocationWhenInUseUsageDescription` keys to your project's **ios/[app]/Info.plist**:
...
**If you're using [CNG](/workflow/continuous-native-generation.md)**, the required `UIBackgroundModes` configuration will be applied automatically by prebuild.
...
-   Background location task must be defined in the top-level scope, using [`TaskManager.defineTask`](/versions/latest/sdk/task-manager.md#taskmanagerdefinetasktaskname-taskexecutor).
-   iOS `"location"` background mode must be specified in **Info.plist** file. See [Background location configuration](/versions/latest/sdk/location.md#background-location-configuration).
-   iOS You must use a [development build](/develop/development-builds/introduction.md) to use background location since it is not supported in the Expo Go app.
...
-   Location permissions must be granted.
-   The Geofencing task must be defined in the top-level scope, using [`TaskManager.defineTask`](/versions/latest/sdk/task-manager.md#taskmanagerdefinetasktaskname-taskexecutor).
...
-   On iOS, it must be granted with the `Always` option using [`requestBackgroundPermissionsAsync`](/versions/latest/sdk/location.md#locationrequestbackgroundpermissionsasync).
...
-   [`requestForegroundPermissionsAsync`](/versions/latest/sdk/location.md#locationrequestforegroundpermissionsasync) maps to `When In Use`
-   [`requestBackgroundPermissionsAsync`](/versions/latest/sdk/location.md#locationrequestbackgroundpermissionsasync) maps to `Always`
...
If the user selected `Allow Once` and you subsequently call [`requestBackgroundPermissionsAsync`](/versions/latest/sdk/location.md#locationrequestbackgroundpermissionsasync) in the same session, the system **will not show another prompt**. Instead, the request will **silently fail**, and the returned background permission status will be **denied**.
...
Deferred updates are configured through [`LocationTaskOptions`](/versions/latest/sdk/location.md#locationtaskoptions) using the [`deferredUpdatesDistance`](/versions/latest/sdk/location.md#locationtaskoptions), [`deferredUpdatesInterval`](/versions/latest/sdk/location.md#locationtaskoptions) and [`deferredTimeout`](/versions/latest/sdk/location.md#locationtaskoptions) properties.

> Deferred locations apply only when the app is in the background.

## Usage

If you're using the Android Emulator or iOS Simulator, ensure that [Location is enabled](/versions/latest/sdk/location.md#enable-emulator-location).
...
Returns: `Promise<motionactivityobject>`

a promise which fulfills with a [`MotionActivityObject`](#motionactivityobject).
...
| Parameter | Type | Description |
| --- | --- | --- |
| `callback` | [LocationHeadingCallback](/versions/latest/sdk/location.md#locationheadingcallbacklocation) | This function is called on each compass update. It receives an object of type [LocationHeadingObject](#locationheadingobject) as the first argument. |
| `errorHandler`(optional) | [LocationErrorCallback](/versions/latest/sdk/location.md#locationerrorcallbackreason) | This function is called when an error occurs. It receives a string with the error message as the first argument. |
...
| Parameter | Type | Description |
| --- | --- | --- |
| `callback` | [MotionActivityCallback](/versions/latest/sdk/location.md#motionactivitycallbackactivity) | This function is called on each motion activity update. It receives an object of type [`MotionActivityObject`](#motionactivityobject) as the first argument. |
| `errorHandler`(optional) | [LocationErrorCallback](/versions/latest/sdk/location.md#locationerrorcallbackreason) | This function is called if the platform reports an error (for example, when activity recognition permission is denied). It receives a string message as the first argument. |
...
| `callback` | [LocationCallback](/versions/latest/sdk/location.md#locationcallbacklocation) | This function is called on each location update. It receives an object of type [`LocationObject`](#locationobject) as the first argument. |
| `errorHandler`(optional) | [LocationErrorCallback](/versions/latest/sdk/location.md#locationerrorcallbackreason) | This function is called when an error occurs. It receives a string with the error message as the first argument. |
...
| Parameter | Type |
| --- | --- |
| `activity` | [MotionActivityObject](#motionactivityobject) |
...
| Property | Type | Description |
| --- | --- | --- |
| activities | Record<[MotionActivityType](#motionactivitytype), [MotionActivityState](#motionactivitystate)\> | An object keyed by `MotionActivityType`, each value being a `MotionActivityState`. |
| timestamp | `number` | The time at which the activity snapshot was taken, in milliseconds since Unix epoch. |
...
| Property | Type | Description |
| --- | --- | --- |
| confidence | [MotionActivityConfidence](#motionactivityconfidence) | Confidence level of the detection. On Android, each activity type has its own confidence derived from the `DetectedActivity` probability value. On iOS, this is the overall reading confidence from `CMMotionActivityConfidence`, shared by all detected activities. |
| detected | `boolean` | Whether this activity was detected. |
...
> Foreground and background services are not available in Expo Go for Android. Instead, we recommend using a [development build](/develop/development-builds/introduction.md) to avoid limitations.
...
Another example can be stated using [available location accuracies](/versions/latest/sdk/location.md#accuracy). Android defines the approximate location accuracy estimation within about 3 square kilometers, and the precise location accuracy estimation within about 50 meters. For example, if the location accuracy value is [Low](/versions/latest/sdk/location.md#low), you can exclude `ACCESS_FINE_LOCATION` permission. To learn more about levels of location accuracies, see [Android documentation](https://developer.android.com/training/location/permissions#accuracy).

To learn more on how to exclude permission, see [Excluding Android permissions](/guides/permissions.md#android).
