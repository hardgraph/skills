> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/integrations/builtin/core-nodes/n8n-nodes-base.readwritefile.md).
...
For persistent file handling on Cloud, use a cloud storage node such as [AWS S3](/integrations/builtin/app-nodes/n8n-nodes-base.awss3.md), [Google Drive](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md), or [FTP](/integrations/builtin/core-nodes/n8n-nodes-base.ftp.md).
{% endhint %}

### Self-hosted n8n <a href="#self-hosted-n8n" id="self-hosted-n8n"></a>

On self-hosted n8n, by default the node can access any path the n8n process can reach. To restrict access, set the [`N8N_RESTRICT_FILE_ACCESS_TO`](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables/security.md) environment variable to one or more allowed directories (semicolon-separated).

{% hint style="info" %}
**Default changes in n8n 2.0**

Starting in n8n 2.0, `N8N_RESTRICT_FILE_ACCESS_TO` defaults to `~/.n8n-files`. To allow file operations elsewhere, set the variable explicitly. Refer to [n8n 2.0 breaking changes](/changelog/v20-breaking-changes.md#set-default-value-for-n8n_restrict_file_access_to) for details.
{% endhint %}
