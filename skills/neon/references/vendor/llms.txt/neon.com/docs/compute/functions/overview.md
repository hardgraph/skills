...
> Summary: Neon Functions are serverless compute you deploy onto a Neon branch. Host an API, AI agent, real-time server, or webhook handler that runs next to your Postgres data, with DATABASE_URL injected automatically.
...
- **Next to your data.** Same region as the branch, with `DATABASE_URL` (plus [AI Gateway](https://neon.com/docs/ai-gateway/overview) and [Object Storage](https://neon.com/docs/storage/overview) credentials) injected automatically. No cross-region hops, and no credentials to wire up.
