...
url: https://docs.apify.com/integrations/mcp.md
parents:
  - [Apify documentation](https://docs.apify.com/llms.txt)
  - [Integrations](https://docs.apify.com/integrations.md)
  - [AI](https://docs.apify.com/integrations/ai.md)
previous: [Build with connectors](https://docs.apify.com/integrations/mcp-connectors/use-in-actors.md)
next: [Milvus](https://docs.apify.com/integrations/milvus.md)
---
...
Apify MCP server vs MCP connectors

This page covers the Apify MCP server, which exposes Apify Actors as tools to outside AI clients. If you are building an Actor that needs to call third-party MCP servers (such as Notion or Slack) on a user's behalf, see [MCP connectors](https://docs.apify.com/integrations/mcp-connectors.md) instead.

## Apify AI vs the MCP server

The Apify MCP server and [Apify AI](https://docs.apify.com/account/apify-ai.md) share the same Actor search and execution backend, but they serve different audiences:

* *MCP server* - a programmatic interface for external AI agents, IDEs, and CLIs. Use it when you build or operate your own agent.
* *Apify AI* - the conversational interface inside [Apify Console](https://console.apify.com) for users who interact with Apify through a chat UI rather than their own agent.

Actor ranking on both surfaces uses parameters similar to those evaluated by the [Actor quality score](https://docs.apify.com/actors/publishing/quality-score.md), so Actors with higher quality scores tend to rank higher.

## Excluded Actors

The MCP server intentionally excludes two categories of Actors from search and execution:

* *Full-permission Actors* - excluded for security. Running a [full-permission Actor](https://docs.apify.com/actors/running/permissions.md#full-permission-actors) is a decision you approve personally, so an LLM can't make it on your behalf.
* *Rental Actors* - excluded because their subscription-based model doesn't fit the sporadic, on-demand way the MCP server runs Actors.
...
* *Apify API token* - Get your API token from the **API & Integrations** section in [Apify Console](https://console.apify.com/settings/integrations). This token authorizes the MCP server to run Actors on your behalf. Make sure to keep it secure.
* *MCP client* - An AI agent or client that supports Model Context Protocol (MCP) This could be Anthropic's Claude for Desktop, a VS Code extension with MCP support, or any application that implements the MCP specification. The [official MCP documentation](https://modelcontextprotocol.io/clients) maintains a list of compatible clients.
...
*MCP server configuration for other clients*: Use the [UI configuration tool](https://mcp.apify.com/) to select Actors and tools, then copy the configuration to your client.
...
For detailed setup options and troubleshooting, see the [Claude Desktop integration guide](https://docs.apify.com/integrations/claude-desktop.md).
...
For Claude Desktop-specific troubleshooting (tools not loading, connection errors, corrupted cache), see [Claude Desktop troubleshooting](https://docs.apify.com/integrations/claude-desktop.md#troubleshooting).

##### Authentication errors

* *Check your API token*: Verify that your Apify API token is correct. You can find it in the **API & Integrations** section of the [Apify Console](https://console.apify.com/settings/integrations). Without a valid token, the server cannot start Actor runs.
