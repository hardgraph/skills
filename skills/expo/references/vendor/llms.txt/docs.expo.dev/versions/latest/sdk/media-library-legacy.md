...
sourceCodeUrl: 'https://github.com/expo/expo/tree/sdk-57/packages/expo-media-library/src/legacy'
...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/sdk/media-library-legacy/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/sdk/media-library-legacy/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
If you're not using Continuous Native Generation ([CNG](/workflow/continuous-native-generation.md)) or you're using native **android** and **ios** projects manually, then you need to add following permissions and configuration to your native projects:
...
Type: React.[Element](https://www.typescriptlang.org/docs/handbook/jsx.html#function-component)<[AlbumsOptions](/versions/latest/sdk/media-library-legacy.md#albumsoptions)\>
...
| --- | --- | --- |
| `assets` | [AssetRef](/versions/latest/sdk/media-library.md#asset) | [AssetRef[]](/versions/latest/sdk/media-library#asset) | An array of [Asset](#asset) or their IDs. |
| `album` | [AlbumRef](/versions/latest/sdk/media-library.md#album) | An [Album](#album) or its ID. |
...
| `asset`(optional) | [AssetRef](/versions/latest/sdk/media-library.md#asset) | An [Asset](#asset) or its ID. On Android you either need to provide an asset or a localUri. |
...
| `album`(optional) | [AlbumRef](/versions/latest/sdk/media-library.md#album) | An [Album](#album) or its ID. If provided, the asset will be added to this album upon creation, otherwise it will be added to the default album for the media type. The album has exist. |

  

Creates an asset from existing file. The most common use case is to save a picture taken by [Camera](/versions/latest/sdk/camera.md). This method requires `CAMERA_ROLL` permission.
...
| Parameter | Type | Description |
| --- | --- | --- |
| `albums` | [AlbumRef](/versions/latest/sdk/media-library.md#album) | [AlbumRef[]](/versions/latest/sdk/media-library#album) | An array of [`Album`](#asset)s or their IDs. |
...
| Parameter | Type | Description |
| --- | --- | --- |
| `asset` | [AssetRef](/versions/latest/sdk/media-library.md#asset) | An [`Asset`](#asset) or its ID. |

  

Returns the `content://` URI for the given legacy asset. Use this when migrating to the new class-based API — pass the returned URI as the ID to `new Asset(id)`.

Returns: `Promise<string>`

A promise which fulfils with the `content://` URI string for the asset.
...
| --- | --- | --- |
| `asset` | [AssetRef](/versions/latest/sdk/media-library.md#asset) | An [Asset](#asset) or its ID. |
| `options`(optional) | [MediaLibraryAssetInfoQueryOptions](/versions/latest/sdk/media-library-legacy.md#medialibraryassetinfoqueryoptions) | - |
...
| Parameter | Type |
| --- | --- |
| `assetsOptions`(optional) | [AssetsOptions](/versions/latest/sdk/media-library-legacy.md#assetsoptions) |
...
A promise that fulfils with to [`PagedInfo`](/versions/v57.0.0/sdk/media-library-legacy.md#pagedinfo) object with array of [`Asset`](#asset)s.
...
| --- | --- | --- |
| approximateLocation(optional) | [Location](/versions/latest/sdk/media-library.md#location) | Supported platforms: iOS. Apply only to albums whose type is `'moment'`. Approximated location of all assets in moment. |
| assetCount | `number` | Estimated number of assets in album. |
| endTime | `number` | Supported platforms: iOS. Apply only to albums whose type is `'moment'`. Latest creation timestamp of all assets in moment. |
| id | `string` | Album ID. |
| locationNames(optional) | `string[]` | Supported platforms: iOS. Apply only to albums whose type is `'moment'`. Names of locations grouped in moment. |
| startTime | `number` | Supported platforms: iOS. Apply only to albums whose type is `'moment'`. Earliest creation timestamp of all assets in moment. |
...
Type: [Asset](/versions/latest/sdk/asset.md#asset) extended by:
...
| location(optional) | [Location](/versions/latest/sdk/media-library.md#location) | GPS location if available. |
...
| pairedVideoAsset(optional) | [Asset](/versions/latest/sdk/asset.md#asset) | null | Supported platforms: iOS. Contains information about the video paired with the image file. This field is available if the `mediaType` is `"photo"`, and the `mediaSubtypes` includes `"livePhoto"`. |
...
| Property | Type | Description |
| --- | --- | --- |
| after(optional) | [AssetRef](/versions/latest/sdk/media-library.md#asset) | Asset ID of the last item returned on the previous page. To get the ID of the next page, pass [`endCursor`](/versions/v57.0.0/sdk/media-library-legacy.md#pagedinfo) as its value. |
| album(optional) | [AlbumRef](/versions/latest/sdk/media-library.md#album) | [Album](#album) or its ID to get assets from specific album. |
