...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/eas-update/introduction/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/eas-update/introduction/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
You are here: EAS > EAS Update
Pages in this section:
- [Introduction](https://docs.expo.dev/eas-update/introduction.md) (this page)
- [Get started](https://docs.expo.dev/eas-update/getting-started.md)
...
**EAS Update** is a cloud service from EAS (Expo Application Services) that serves updates for projects using the [`expo-updates`](/versions/latest/sdk/updates.md) library.
...
For complete steps to start using EAS Update, see [Get started with EAS Update](/eas-update/getting-started.md).

### Expo Skills for AI agents

If you use an AI agent, install [Expo Skills](/skills.md) to teach it how to check the health of your published updates:

[eas-update-insights](https://github.com/expo/skills/blob/main/plugins/expo/skills/eas-update-insights/SKILL.md) — Check the health of published EAS Update: crash rates, install/launch counts, unique users, payload size, and the split between embedded and OTA users per channel.

## Key features

### JS API for update management

The updates [JavaScript API](/versions/latest/sdk/updates.md) includes a React hook called `useUpdates()`. This hook provides detailed information about the currently running update and any new updates that are available or have been downloaded. In addition, you can view any errors that were encountered during the update process to help you debug any issues while the app is attempting to update.
...
You'll get a [deployments dashboard](https://expo.dev/accounts/%5Baccount%5D/projects/%5Bproject%5D/deployments) that helps visualize which updates are being sent to builds. Updates work in concert with [insights](/eas-insights/introduction.md) to provide data on the adoption rates of your updates with your users.

### Republish for reverting mistakes

If an update isn't performing as expected, you can [republish](/eas-update/eas-cli.md#republish-a-previous-update-within-a-branch) a previous, stable version on top of the problematic one, much like a new "commit" in version control systems.
...
| Roll out changes to a percentage of users using [rollouts](/eas-update/rollouts.md) | ✓ |
| Publish updates from [CI or automated workflows](/eas/workflows/pre-packaged-jobs.md#update) | ✓ |
...
For scenarios marked with ✗, use [EAS Build](/build/introduction.md) to create and submit a new app binary.
...
By default, `expo-updates` checks for updates every time the app is loaded. You can implement a custom update strategy with the [Updates API](/versions/latest/sdk/updates.md) and [app config](/versions/latest/config/app.md#updates).

#### Can I use EAS Update with my existing React Native project?

Yes. EAS Update works with both projects that use [Continuous Native Generation (CNG)](/workflow/continuous-native-generation.md) and [existing React Native projects](/bare/installing-updates.md) that have the [`expo-updates`](/versions/latest/sdk/updates.md) library installed.
...
EAS Update uses [runtime version policies](/eas-update/runtime-versions.md) to ensure updates are only sent to builds with compatible native code. If your native code changes, you create a new runtime version.

#### Can I use EAS Update inside EAS Workflows or from other CI/CD pipelines?

Yes. EAS Update works with [EAS Workflows](/eas/workflows/get-started.md). You still need to configure and create a new build for Android or iOS. After that, you can add an update job to your workflow configuration. For example:
...
For more information, see [EAS Workflows pre-packaged jobs](/eas/workflows/pre-packaged-jobs.md#update).

To automate publishing updates with GitHub Actions, see the [GitHub Action for PR previews](/eas-update/github-actions.md) guide.

#### What's the difference between EAS Update and CodePush?

EAS Update is a native solution that also integrates with [EAS Build](/build/introduction.md) and provides a unified workflow. CodePush uses a slightly different approach. To learn more about the differences between EAS Update and CodePush, see [Conceptual differences between CodePush and EAS Update](/eas-update/codepush.md#conceptual-differences-between-codepush-and-eas-update).
...
We recommend transitioning to EAS Update or using a [self-hosted update service](/versions/latest/sdk/updates.md).

## Get started

[Get started with EAS Update](/eas-update/getting-started.md) — Learn how to get started with the setup required to configure and use EAS Update in your project.

[Publish an update](/eas-update/getting-started.md#publish-an-update) — Learn how to publish an update to a specific branch with EAS Update.

[Preview updates](/eas-update/preview.md) — View your teammate's changes with EAS Update.

[Use GitHub Actions](/eas-update/github-actions.md) — Publish an update and preview with a QR code after a commit.

[Migrate from CodePush](/eas-update/codepush.md) — Learn how to migrate from CodePush to EAS Update.

[Using EAS Update with other EAS services](/tutorial/eas/introduction.md) — For a complete tutorial of using EAS Update with other EAS services, refer to this EAS tutorial.
