...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/router/migrate/from-react-navigation/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/router/migrate/from-react-navigation/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [React Navigation](https://docs.expo.dev/router/migrate/from-react-navigation.md) (this page)
...
> This guide targets **SDK 56 and later**. In SDK 56, Expo Router stopped accepting application-code imports from `@react-navigation/*. They now come from` expo-router/\*\` entry points. If you are upgrading an existing Expo Router app from SDK 55 or earlier, follow the [SDK 55 to 56 migration guide](/router/migrate/sdk-55-to-56.md). The samples below already use the SDK 56 and later import paths.

## Pitch

Along with all the benefits of React Navigation, Expo Router enables automatic deep linking, [type safety](/router/reference/typed-routes.md), [deferred bundling](/router/web/async-routes.md), [static rendering on web](/router/web/static-rendering.md), and more.

## Anti-pitch

If your app uses a custom `getPathFromState` or `getStateFromPath` component, it may not be a good fit for Expo Router. If you're using these functions to support [shared routes](/router/advanced/shared-routes.md) then you should be fine as Expo Router has built-in support for this.
...
-   Convert the project to [TypeScript](/guides/typescript.md#migrating-existing-javascript-project). This will make it easier to spot errors that may occur during the migration.
-   Convert relative imports to [typed aliases](/guides/typescript.md#path-aliases-optional). For example, `../../components/button.tsx` to `@/components/button` before starting the migration. This makes it easier to move screens around the filesystem without having to update the relative paths.
...
On web, returning `null` from the root will cause [static rendering](/router/web/static-rendering.md) to skip all of the children, resulting in no searchable content. This can be tested by using "View Page Source" in Chrome, or by disabling JavaScript and reloading the page.
...
Layout the structure of your app by creating files according to the [application of Expo Router rules](/router/basics/core-concepts.md#the-rules-of-expo-router-applied). Kebab-case and lowercase letters are considered best practice for route filenames.
...
Similarly, migrate from the `route` prop to the [`useLocalSearchParams`](/versions/latest/sdk/router.md#uselocalsearchparams) hook.

To access the [`navigation.navigate`](https://reactnavigation.org/docs/navigation-object/#navigate), import the `navigation` prop from [`useNavigation`](/versions/latest/sdk/router.md#usenavigationparent) hook.
...
In Expo Router, you can either migrate to [shared routes](/router/advanced/shared-routes.md) or create multiple files and re-export the same component from them.
...
You may have your screen tracking setup according to our [React Navigation screen tracking guide](https://reactnavigation.org/docs/screen-tracking/), update it according to the [Expo Router screen tracking guide](/router/reference/screen-tracking.md).

### Use platform-specific components for screens

Refer to the [platform-specific modules](/router/advanced/platform-specific-modules.md) guide for info on switching UI based on the platform.
...
Unlike React Navigation, Expo Router can reliably represent any route with a string. Use the [`usePathname()`](/versions/latest/sdk/router.md#usepathname) or [`useSegments()`](/versions/latest/sdk/router.md#usesegments) hooks to identify the current route.

#### `getCurrentOptions`

Use the [`useLocalSearchParams()`](/versions/latest/sdk/router.md#uselocalsearchparams) hook to get the current route's query parameters.
...
Use the [`usePathname()`](/versions/latest/sdk/router.md#usepathname) or [`useSegments()`](/versions/latest/sdk/router.md#usesegments) hooks to identify the current route. Use in conjunction with `useEffect(() => {}, [...])` to observe changes.

#### `options`

Use the [`useLocalSearchParams()`](/versions/latest/sdk/router.md#uselocalsearchparams) hook to get the current route's query parameters. Use in conjunction with `useEffect(() => {}, [...])` to observe changes.
...
In Expo Router, you can rehydrate your application state from a route string (for example, `/user/evanbacon`). Use [redirects](/router/reference/redirects.md) to handle initial states. See [shared routes](/router/advanced/shared-routes.md) for advanced redirects.
...
Use the [`usePathname()`](/versions/latest/sdk/router.md#usepathname), [`useSegments()`](/versions/latest/sdk/router.md#usesegments), and [`useGlobalSearchParams()`](/versions/latest/sdk/router.md#useglobalsearchparams) hooks to identify the current route state. Use in conjunction with `useEffect(() => {}, [...])` to observe changes.

-   If you're attempting to track screen changes, follow the [Screen Tracking guide](/router/reference/screen-tracking.md).
...
-   See the [Screen Tracking guide](/router/reference/screen-tracking.md) for info on migrating analytics from React Navigation.
-   See the [Splash Screen feature](/develop/user-interface/splash-screen-and-app-icon.md) for info on handling the splash screen.

#### `onUnhandledAction`

Actions are always handled in Expo Router. Use [dynamic routes](/router/basics/notation.md#square-brackets) and [404 screens](/router/error-handling.md#unmatched-routes) in favor of [`onUnhandledAction`](https://reactnavigation.org/docs/navigation-container/#onunhandledaction).
...
The [`fallback`](https://reactnavigation.org/docs/navigation-container/#fallback) prop is automatically handled by Expo Router. Learn more in the [Splash Screen](/versions/latest/sdk/splash-screen.md) reference.
...
Use the [Head component](/router/web/static-rendering.md#meta-tags) to set the webpage title.
...
If you're observing the navigation state directly, migrate to the [`usePathname`](/versions/latest/sdk/router.md#usepathname), [`useSegments`](/versions/latest/sdk/router.md#usesegments), and [`useGlobalSearchParams`](/versions/latest/sdk/router.md#useglobalsearchparams) hooks.
...
In React Navigation, you can use the `initialRouteName` property of the linking configuration. In Expo Router, use [layout settings](/router/advanced/router-settings.md).

### Reset navigation state

You can use the [`reset`](https://reactnavigation.org/docs/navigation-actions/#reset) action from the React Navigation library to reset the navigation state. It is dispatched using the [`useNavigation`](/versions/latest/sdk/router.md#usenavigationparent) hook from Expo Router to access the `navigation` prop.
...
Expo Router can automatically generate [statically typed routes](/router/reference/typed-routes.md), this will ensure you can only navigate to valid routes.
