> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/changelog/release-notes-0.x.md).
...
For a curated summary of the changes that matter most, see the [Changelog](/changelog/readme.md).
{% endhint %}
...
[crowd.dev node documentation](broken://spaces/BKcbOzIWja8NfqKDcqHc/pages/ToWwEdQ8fffhdLOqDsk7).
{% endhint %}
...
Refer to [Node user interface elements | Resource mapper](/connect/create-nodes/build-your-node/reference/node-ui-elements.md#resource-mapper) for guidance for node builders.
...
* Running n8n in own mode. Main mode is now the default. Use [Queue mode](/deploy/host-n8n/configure-n8n/scaling/enable-queue-mode.md) if you need full execution isolation.
...
{% hint style="info" %}
This release introduces [Variables](/build/code-in-n8n/define-custom-variables.md). You can now create variables that allows you to store and reuse values in n8n workflows. This is the first phase of a larger project to support [Environments](/administer/use-source-control-and-environments.md) in n8n.
{% endhint %}
...
{% hint style="info" %}
This release introduces improvements to the execution lists. You can now save [Custom execution data](/build/understand-workflows/understand-executions/customize-executions-data.md), and use it to filter both the [All executions](/build/understand-workflows/understand-executions/view-all-executions.md) and [Single workflow executions](/build/understand-workflows/understand-executions/view-executions-for-a-single-workflow.md) lists.
{% endhint %}
...
* Core: add configurable execution history limit. Use this to improve performance when self-hosting. Refer to [Execution Data | Enable data pruning ](/deploy/host-n8n/configure-n8n/scaling/manage-execution-data.md#enable-executions-pruning)for more information.
...
* Editor: support mapping keys with drag and drop. Any field with the hint **Enter the field name as text** should now support mapping a data key using drag and drop. Node builders can enable this in their own nodes. Refer to [Creating nodes | UI elements](/connect/create-nodes/build-your-node/reference/node-ui-elements.md#support-drag-and-drop-for-data-keys) for more information.
* Editor: add the [HTML editor component](/connect/create-nodes/build-your-node/reference/node-ui-elements.md#html) for use in parameters. This means node builders can now use the HTML editor that n8n uses in the HTML node as a UI component.
...
This release introduces support for LDAP on Self-hosted Enterprise and Cloud Enterprise plans. Refer to [LDAP](/administer/manage-users-and-access/verify-user-identity/connect-ldap.md) for more information on this feature.
{% endhint %}
...
This release adds experimental support for more Prometheus metrics. Self-hosting users can configure Prometheus using [environment variables](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables.md).
...
This release adds a new [Google Sheets Trigger node](/integrations/builtin/trigger-nodes/n8n-nodes-base.googlesheetstrigger.md). You can now start workflows in response to row changes or new rows in a Google Sheet.
{% endhint %}
...
When using [expressions](/build/work-with-data/expressions-versus-data-nodes.md), n8n will now offer you suggestions as you type.

<img src="/files/XfmLhLEayds5SB5hQ6sJ" alt="&#x22;Animated GIF showing typeahead in action&#x22;" data-size="original">
{% endhint %}
...
This release introduces [log streaming](/administer/observe-and-log/stream-logs-to-external-systems.md) for users on Enterprise self-hosted plans and custom Cloud plans. Log streaming allows you to send events from n8n to your own logging tools. This allows you to manage your n8n monitoring in your own alerting and logging processes.
{% endhint %}
...
This release adds a [security audit](/deploy/host-n8n/configure-n8n/security/run-security-audits.md) feature. You can now run a security audit on your n8n instance, to detect common security issues.
{% endhint %}
...
This release introduces workflow sharing for users on some plans. With workflow sharing, users can invite other users on the same n8n instance to use and edit their workflows. Refer to [Workflow sharing](/build/manage-workflows/share-with-others.md) for details.
{% endhint %}
...
<img src="/files/wW4QjdPR4Gs8mZQDedIS" alt="&#x22;Screenshot of the inline expressions editors&#x22;" data-size="original">
{% endhint %}
...
This release adds an integration with OpenAI. Refer to the [OpenAI node documentation](/integrations/builtin/app-nodes/n8n-nodes-langchain.openai.md) for details.
{% endhint %}
...
* Editor: when building nodes, you can now add a property with type `notice` to your credentials `properties`.This was previously available in nodes but not credentials. Refer to [Node UI elements](/connect/create-nodes/build-your-node/reference/node-ui-elements.md) for more information.
...
This release introduces the Schedule Trigger node, replacing the Cron node. You can find documentation for the new node [here](/integrations/builtin/core-nodes/n8n-nodes-base.scheduletrigger.md).
{% endhint %}

### Node enhancements <a href="#node-enhancements" id="node-enhancements"></a>

* HubSpot node: you can now use your HubSpot credentials in the HTTP Request node to make a [custom API call](/integrations/builtin/custom-api-actions-for-existing-nodes.md).
* Rundeck node: you can now use your Rundeck credentials in the HTTP Request node to make a [custom API call](/integrations/builtin/custom-api-actions-for-existing-nodes.md).
...
This release introduces the Code node. This node replaces both the Function and Function Item nodes. Refer to the [Code node](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) documentation for more information.
{% endhint %}
...
Introducing improved support for item linking (paired items). Item linking is a key concept in the n8n data flow. Learn more in [Data item linking](/build/work-with-data/reference-data/link-data-items.md).
{% endhint %}
...
n8n's [built-in methods and variables](/build/code-in-n8n.md) have been overhauled, introducing new variables, and providing greater consistency in behavior and naming.
{% endhint %}
...
The [WhatsApp Business Platform](/integrations/builtin/app-nodes/n8n-nodes-base.whatsapp.md) node allows you to use the WhatsApp Business Platform Cloud API in your workflows.
{% endhint %}

### Node enhancements <a href="#node-enhancements" id="node-enhancements"></a>

* HTTP Request node: a major overhaul. It's now much simpler to build a custom API request. Refer to the [HTTP Request node documentation](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) for more information.
...
This release adds support for n8n's existing [user management](/administer/manage-users-and-access.md) functionality to Cloud, and introduces a new feature: [credential sharing](/administer/manage-credentials/share-credentials-securely.md). Credential sharing is currently only available on Cloud.
{% endhint %}

Also in this release:

* Added a `resourceLocator` parameter type for nodes, and started upgrading n8n's built-in nodes to use it. This new option helps users who need to specify the ID of a record or item in an external service. For example, when using the Trello node, you can now search for a specific card by ID, URL, or do a free text search for card titles. Node builders can learn more about working with this new UI element in n8n's [UI elements](/connect/create-nodes/build-your-node/reference/node-ui-elements.md) documentation.
...
n8n now has a [Gmail Trigger node](/integrations/builtin/trigger-nodes/n8n-nodes-base.gmailtrigger.md). This allows you to trigger workflows in response to a Gmail account receiving an email.
{% endhint %}

### Node enhancements <a href="#node-enhancements" id="node-enhancements"></a>

* Gmail node: this release includes an overhaul of the [Gmail node](/integrations/builtin/app-nodes/n8n-nodes-base.gmail.md), with updated resources and operations.
...
n8n\@0.187.0 saw the first release of [data mapping](/build/work-with-data/reference-data.md), allowing you to drag and drop top level data from a node's **INPUT** panel into parameter fields. With this release, you can now drag and drop data from any level.
{% endhint %}
...
This release introduces the [community node repository](/integrations/community-nodes/installation-and-management.md). This allows developers to build and share nodes as npm packages. Users can install community-built nodes directly in n8n.
{% endhint %}
...
This release introduces a drag and drop interface for [data mapping](/build/work-with-data/reference-data.md), as a quick way to map data without using expressions.
{% endhint %}
...
* [Google Drive node](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md): add support for move to trash.
* [Mindee node](/integrations/builtin/app-nodes/n8n-nodes-base.mindee.md): support new version.
* [Notion node](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md): support ignoring the Notion URL property if empty.
...
* [Postgres node](/integrations/builtin/app-nodes/n8n-nodes-base.postgres.md): improve handling of large numbers.
...
* [Google Drive node](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md): process input items using the list operation.
* [Telegram node](/integrations/builtin/app-nodes/n8n-nodes-base.telegram.md): fix for a bug affecting sending binary data (images, documents and so on).
...
* [HTTP Request node](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md): ensure all OAuth2 credentials work with this node.
...
* [HTTP Request node](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md): better handling for requests that return null.
...
This release introduces the n8n public REST API. Using n8n's public API, you can programmatically perform many of the same tasks as you can in the GUI. The API includes a built-in Swagger UI playground. Refer to the [API documentation](/connect/n8n-api.md) for more information.
{% endhint %}
...
* [Discord node](/integrations/builtin/app-nodes/n8n-nodes-base.discord.md): Fixed rate limit handling.
* [Gmail node](/integrations/builtin/app-nodes/n8n-nodes-base.gmail.md): Fixed sending attachments in filesystem data mode.
* [Google Sheets node](/integrations/builtin/app-nodes/n8n-nodes-base.googlesheets.md): Fixed an error preventing the *Use Header Names as JSON Paths* option from working as expected.
...
For more information, refer to [Custom API operations](/integrations/builtin/custom-api-actions-for-existing-nodes.md).
...
* [Google Drive Node](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md): Drive upload, delete, and share operations now support shared Drives.
...
* [Google Calendar Node](/integrations/builtin/app-nodes/n8n-nodes-base.googlecalendar.md): Make it work with public calendars and clean up.
...
* [Google Sheets node](/integrations/builtin/app-nodes/n8n-nodes-base.googlesheets.md): n8n now handles header names formatted as JSON paths.
...
* [Telegram node](/integrations/builtin/app-nodes/n8n-nodes-base.telegram.md): add support for querying chat administrators.
...
This release adds Sticky Notes, a new feature that allows you to annotate and comment on your workflows. Refer to the [Sticky Notes](/build/understand-workflows/workflow-components/add-notes-and-documentation.md) for more information.
{% endhint %}
...
* [Google Sheets node](/integrations/builtin/app-nodes/n8n-nodes-base.googlesheets.md): Added upsert support.
...
* [Discord node](/integrations/builtin/app-nodes/n8n-nodes-base.discord.md): additional fields now available when sending a message to Discord.
...
**core**: Luxon now applies the correct timezone. Refer to [Luxon](/build/work-with-data/handle-special-data-types/work-with-dates-and-times.md) for more information.\
...
* [HTTP Request Node:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Add support for OPTIONS method.
...
* [Supabase Node:](/integrations/builtin/app-nodes/n8n-nodes-base.supabase.md) Send token also using Authorization Bearer; fix Row > Get operation.
...
Read more about the new license in [License](/privacy-and-security/sustainable-use-license.md).
...
* [HTTP Request Node:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Allow Delete requests with body.
...
* [Telegram node:](/integrations/builtin/app-nodes/n8n-nodes-base.telegram.md) Fix sending binary data when disable notification is set.
...
Check out the [user management documentation](/administer/manage-users-and-access.md) for more information.
...
* [Function:](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) Added support for items without a JSON key.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Fixed a bug causing Digest Authentication to fail in some scenarios.
...
* [Notion (Beta):](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Added support for rollup fields to the Simplify Output option. Also added the Parent ID to the Get All operation of the Block resource.
...
* [Telegram Trigger:](/integrations/builtin/trigger-nodes/n8n-nodes-base.telegramtrigger.md) Added new Extra Large option to Image Size field.
* [Webhook:](/integrations/builtin/core-nodes/n8n-nodes-base.webhook.md) Added new No Response Body option. Also added support for DELETE, PATCH and PUT methods.
...
* [Function:](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) Fixed a bug leaving the code editor size unchanged after resizing the window.
* [Function Item:](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) Fixed a bug leaving the code editor size unchanged after resizing the window.
...
* [Airtable:](/integrations/builtin/app-nodes/n8n-nodes-base.airtable.md) Improved field description.
...
* [Gmail:](/integrations/builtin/app-nodes/n8n-nodes-base.gmail.md) Fixed a bug causing the BCC field to be ignored.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Fixed a bug causing custom headers from being ignored.
...
* [Supabase:](/integrations/builtin/app-nodes/n8n-nodes-base.supabase.md) Fixed an errorneous documentation link.
...
* [Function:](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) The node now prevents unsupported data from being returned.
* [Function Item:](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) The node now prevents unsupported data from being returned.
...
* [Notion (Beta):](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Upgraded the Notion node: Added Search operation for the Database resource, Get operation for Database Page resource, Archive operation for the Page resource. Also added Simplify Output option and test for credential validity.
* [Wait:](/integrations/builtin/core-nodes/n8n-nodes-base.wait.md) Added new Ignore Bots option.
* [Webhook:](/integrations/builtin/core-nodes/n8n-nodes-base.webhook.md) Added new Ignore Bots option.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Fixed a bug where using Digest Auth would fail.
...
* [Google Drive:](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md) Added support for downloading and converting native Google files.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Added support for query-based authentication.
...
* [Telegram Trigger:](/integrations/builtin/trigger-nodes/n8n-nodes-base.telegramtrigger.md) Added support for downloading images to channel\_post updates.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Fixed an issue causing the wrong Content-Type header to be set when downloading a file.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Fixed an issue where the Content-Length header was not calculated and sent when using the a Body Content Type of Form-Data Multipart.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Fixed an issue with the Full Response option not taking effect when used with the Ignore Response Code option.
...
* [Airtable:](/integrations/builtin/app-nodes/n8n-nodes-base.airtable.md) Fixed an issue with the sort field.
* [Cron:](/integrations/builtin/core-nodes/n8n-nodes-base.scheduletrigger.md) Set the version of the cron library to 1.7.2.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Fixed 'Ignore response code' flag.
...
* [Telegram:](/integrations/builtin/app-nodes/n8n-nodes-base.telegram.md) Added binary data support to the Send Animation, Send Audio, Send Document, Send Photo, Send Video, and Send Sticker operations.
...
* [Notion:](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Fixed issue listed unnamed databases.
...
* [Notion:](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Added handling of Rich Text when simplifying data.
...
* [Webhook:](/integrations/builtin/core-nodes/n8n-nodes-base.webhook.md) Fixed issue when receiving multiple files.
...
* [Interval:](/integrations/builtin/core-nodes/n8n-nodes-base.scheduletrigger.md) Fixed issue where entering too large a value (> 2147483647ms) resulted in an interval of 1sec being used rather than an error.
...
* [Google Drive:](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md) Added File Name option to File > Update operation.
...
* [Notion:](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Added Timezone option when inserting Date fields.
...
* Integrated [Nodelinter](/connect/create-nodes/test-your-node/node-linter.md) in n8n.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Added Follow All Redirects option.
...
* [Interval:](/integrations/builtin/core-nodes/n8n-nodes-base.scheduletrigger.md) Fixed issue with infinite executions.
...
* [Facebook Trigger:](/integrations/builtin/trigger-nodes/n8n-nodes-base.facebooktrigger.md) Added Fields parameter.
* [Gmail:](/integrations/builtin/app-nodes/n8n-nodes-base.gmail.md) Added Sender Name parameter.
...
* [Notion:](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Fixed issue when filtering field data type.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Added support for arrays in Querystring. Any parameter appearing multiple times with the same name is grouped into an array.
* [Mautic:](/integrations/builtin/app-nodes/n8n-nodes-base.mautic.md) Added Contact Segment resource.
* [Telegram:](/integrations/builtin/app-nodes/n8n-nodes-base.telegram.md) Added Delete operation to the Message resource.
...
* [Airtable:](/integrations/builtin/app-nodes/n8n-nodes-base.airtable.md) Added Bulk Size option to all Operations.
...
* [Google Sheets:](/integrations/builtin/app-nodes/n8n-nodes-base.googlesheets.md) Fixed an issue with handling 0 value that resulted in empty cells.
...
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Added Split Into Items option.
...
* Added new Editor UI shortcuts. See [Keyboard Shortcuts](/build/keyboard-shortcuts.md) for details.
...
* [Notion:](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Fixed an issue with pagination of databases and users.
...
* [MySQL:](/integrations/builtin/app-nodes/n8n-nodes-base.mysql.md) Fixed an issue where n8n was unable to save data due to collation, resulting in workflows ending with Unknown status.
...
* [Google Drive:](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md) Added APP Properties and Properties options to the Upload operation of the File resource
* [HTTP Request:](/integrations/builtin/core-nodes/n8n-nodes-base.httprequest.md) Added the functionality to log the request to the browser console for testing
* [Notion:](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Added the Include Time parameter date field types
...
* [Notion:](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Fixed an issue with parsing the last edited time
...
* [Function:](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) Added console.log support for writing to browser console
* [Function Item:](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) Added console.log support for writing to browser console
...
* [Function:](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) Added the functionality to log console.log messages to the browser console
* [Function Item:](/integrations/builtin/core-nodes/n8n-nodes-base.code.md) Added the functionality to log console.log messages to the browser console
...
* [Google Drive:](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md) Fixed an issue with the Drive resource
* [Notion:](/integrations/builtin/app-nodes/n8n-nodes-base.notion.md) Fixed an issue with the filtering fields type and fixed an issue with the link option
...
* [Google Sheets:](/integrations/builtin/app-nodes/n8n-nodes-base.googlesheets.md) Fixed an issue with the Key Row parameter
...
* [Google Calendar:](/integrations/builtin/app-nodes/n8n-nodes-base.googlecalendar.md) Added the Continue on Fail functionality
* [Google Drive:](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md) Added the functionality to add file name when downloading files
* [Gmail:](/integrations/builtin/app-nodes/n8n-nodes-base.gmail.md) Added functionality to handle multiple binary properties
...
* [Postgres:](/integrations/builtin/app-nodes/n8n-nodes-base.postgres.md) Added query parameters. The Execute Query operation returns the result from all queries executed instead of just one of the results.
...
* [Google Sheets:](/integrations/builtin/app-nodes/n8n-nodes-base.googlesheets.md) Fixed an issue with the Lookup operation for returning empty rows
...
* [Postgres:](/integrations/builtin/app-nodes/n8n-nodes-base.postgres.md) Added the Mode option that allows you to execute queries as transactions
...
* [Google Sheets:](/integrations/builtin/app-nodes/n8n-nodes-base.googlesheets.md) Fixed an issue with the Lookup operation for returning multiple empty rows
...
* [MySQL:](/integrations/builtin/app-nodes/n8n-nodes-base.mysql.md) Added support for insert modifiers and added support for SSL
...
* [Google Drive:](/integrations/builtin/app-nodes/n8n-nodes-base.googledrive.md) Added support for creating folders for shared drives
* [Google Sheets:](/integrations/builtin/app-nodes/n8n-nodes-base.googlesheets.md) Added Create and Remove operation to the Sheet resource
...
* [Postgres:](/integrations/builtin/app-nodes/n8n-nodes-base.postgres.md) Added support for type casting
...
* Added the functionality to expose metrics to Prometheus. Read more about that [here](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables.md)
...
* Added CLI commands to [export](/deploy/host-n8n/configure-n8n/use-the-command-line.md#export-workflows-and-credentials) and [import](/deploy/host-n8n/configure-n8n/use-the-command-line.md#import-workflows-and-credentials) credentials and workflows
