...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/billing/usage-based-pricing/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/billing/usage-based-pricing/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Usage-based pricing](https://docs.expo.dev/billing/usage-based-pricing.md) (this page)
...
Expo applies usage-based billing for customers who exceed their [plan](/billing/plans.md) allowances. This enables our customers to use what they need without worrying about limitations or requiring contractual obligations.
...
This is because Expo only charges for [unique monthly active users](/eas-update/introduction.md#how-are-monthly-active-users-counted-for). As a result, the subscriber's bill for extra usage will be:
...
Expo sends email notifications to account [Owners and Admins](/accounts/account-types.md#manage-access) when your account reaches **80%** and **100%** of your plan's included EAS Build credits. This helps you anticipate overage charges before they occur.
...
You can use `npx expo-updates assets:verify <dir>` to check all required assets are included in the update. For more information, see [Asset selection and exclusion](/eas-update/asset-selection.md).
