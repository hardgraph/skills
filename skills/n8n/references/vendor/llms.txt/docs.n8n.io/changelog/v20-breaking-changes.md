> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/changelog/v20-breaking-changes.md).
...
Parent-Workflow: ![Parent-Workflow](/files/lY1W1IGSJRojueotwId6)

Sub-Workflow: ![Sub-Workflow](/files/cXuDxqo4KdtkYyxWbmu1)

v1: The parent-execution reproduces the sub-execution's input as its output.: ![v1: Parent execution won't receive the result of the child execution](/files/tA4GOnFnOxvNdUYohcxK)

v2: The parent execution receives the result of the child execution: ![v2: Parent execution will receive the result of the child execution](/files/T1kWARjn63KwoktDO3zW)
...
n8n will enable [task runners](/deploy/host-n8n/configure-n8n/set-up-task-runners.md) by default to improve security and isolation. All Code node executions will run on task runners.

**Migration path:** Before upgrading to v2.0, set N8N\_RUNNERS\_ENABLED=true to test this behavior. Make sure your infrastructure meets the requirements for running task runners. For additional security, consider using [external mode](/deploy/host-n8n/configure-n8n/set-up-task-runners.md#external-mode).
...
n8n will remove the Pyodide-based Python Code node and tool and replace them with a [task runner-based](/deploy/host-n8n/configure-n8n/set-up-task-runners.md) implementation that uses native Python for better security and performance. Starting in v2.0, you can only use Python Code nodes with task runners in [external mode](/deploy/host-n8n/configure-n8n/set-up-task-runners.md#external-mode) and native Python tools.

The native Python Code node doesn't support built-in variables like `_input` or dot access notation, which were available in the Pyodide-based version. For details, see the [Code node documentation](/integrations/builtin/core-nodes/n8n-nodes-base.code.md#python-native).
...
### Set default value for N8N\_RESTRICT\_FILE\_ACCESS\_TO <a href="#set-default-value-for-n8nrestrictfileaccessto" id="set-default-value-for-n8nrestrictfileaccessto"></a>
...
### Change the default value of N8N\_GIT\_NODE\_DISABLE\_BARE\_REPOS to true <a href="#change-the-default-value-of-n8ngitnodedisablebarerepos-to-true" id="change-the-default-value-of-n8ngitnodedisablebarerepos-to-true"></a>
...
**Migration path:** Filesystem or database mode will be used automatically based on configuration. Make sure your n8n instance has enough disk space to store binary data. For details, see the [binary data configuration](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables/binary-data.md).
...
### Remove QUEUE\_WORKER\_MAX\_STALLED\_COUNT <a href="#remove-queueworkermaxstalledcount" id="remove-queueworkermaxstalledcount"></a>
...
### Remove N8N\_CONFIG\_FILES <a href="#remove-n8nconfigfiles" id="remove-n8nconfigfiles"></a>

The `N8N_CONFIG_FILES` environment variable has been removed.

**Migration path:** Delete this environment variable from your configuration. Move configuration into environment variables, an `.env` file or [`_FILE`](/deploy/host-n8n/configure-n8n/basic-configuration.md#keeping-sensitive-data-in-separate-files) based configuration.
