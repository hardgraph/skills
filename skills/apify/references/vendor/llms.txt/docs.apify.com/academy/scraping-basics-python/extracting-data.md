...
url: https://docs.apify.com/academy/scraping-basics-python/extracting-data.md
parents:
...
previous: [Locating HTML elements](https://docs.apify.com/academy/scraping-basics-python/locating-elements.md)
next: [Saving data](https://docs.apify.com/academy/scraping-basics-python/saving-data.md)
---
...
We call the operation of removing whitespace *stripping* or *trimming*, and it's so useful in many applications that programming languages and libraries include ready-made tools for it. Let's add Python's built-in [.strip()](https://docs.python.org/3/library/stdtypes.html#str.strip):
...
Download the [IKEA JONAXEL system page](https://www.ikea.com/se/en/cat/jonaxel-system-45730/) and parse it with Beautiful Soup. Print each product's width, depth, height, and price.
...
Simplify the code from the previous exercise. Use a [regular expression](https://docs.python.org/3/library/re.html) to parse the width, depth, and height. Match digits with `[0-9]` or `\d`, and use `+` to match one or more digits.
