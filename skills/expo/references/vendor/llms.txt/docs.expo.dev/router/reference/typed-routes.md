...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/router/reference/typed-routes/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/router/reference/typed-routes/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Typed routes](https://docs.expo.dev/router/reference/typed-routes.md) (this page)
...
> Available when using TypeScript in your project. Expo Router supports standard TypeScript out of the box. See the [TypeScript](/guides/typescript.md) guide for more information on how to set it up.

Expo Router supports generating TypeScript types automatically with Expo CLI. This enables `<Link>`, and the [hooks API](/versions/latest/sdk/router.md#hooks) to be statically typed. This feature is currently in beta and is not enabled by default.

## Get started

### Quick start

If you created your project using the [Expo Router quick start guide](/router/introduction.md#quick-start) then your project is already configured to use typed routes. The Expo CLI will generate the required type file the first time you run `npx expo start`. Then, you can use autocomplete for `href` props whenever you use an Expo Router `<Link>` component in a **.tsx** file.
...
> **Note**: A [`Route` type](/versions/latest/sdk/router.md#route) is also made available by `expo-router`, which matches all valid routes in a project automatically.
