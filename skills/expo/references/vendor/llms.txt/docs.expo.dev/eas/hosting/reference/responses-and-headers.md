...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/eas/hosting/reference/responses-and-headers/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/eas/hosting/reference/responses-and-headers/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Responses and headers](https://docs.expo.dev/eas/hosting/reference/responses-and-headers.md) (this page)
- [Worker runtime](https://docs.expo.dev/eas/hosting/reference/worker-runtime.md)
...
EAS Hosting routes requests from several hostnames to your deployments. [Aliases](/eas/hosting/deployments-and-aliases.md) and [Custom domains](/eas/hosting/custom-domain.md) mean that there may be a difference between the **incoming** URL that clients have used to make a request, and the **target** URL that your API routes receive.
