...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/guides/controlled-components/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/guides/controlled-components/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Controlled components](https://docs.expo.dev/guides/controlled-components.md) (this page)
...
```
1. Keystroke [Native/UI thread]
→ 2. Field shows the raw text [Native/UI thread]
→ 3. onChangeText fires with the string [JavaScript thread]
→ 4. State update, then re-render [JavaScript thread]
→ 5. value forces the field to match [Native/UI thread]
...
Before React Native's [New Architecture](/guides/new-architecture.md), reformatting the value inside `onChangeText` snapped the cursor to the end of the field, which made edits in the middle of the input difficult.
...
-   Prefer native props such as `maxLength` and `editable` over reimplementing them in JavaScript. See [Restrict what users can type](/guides/controlled-components.md#restrict-what-users-can-type).
-   Use a mask library that handles the cursor math for you. See [When to use a library](/guides/controlled-components.md#when-to-use-a-library).
-   Move formatting onto the UI thread. See [Remove formatting flicker with worklets](/guides/controlled-components.md#remove-formatting-flicker-with-worklets).

### Remove formatting flicker with worklets

Formatting in JavaScript round-trips through state and a re-render before the native field updates, so the field can briefly show the raw text before the formatted value replaces it. The universal [`TextInput`](/versions/latest/sdk/ui/universal/textinput.md) from [`@expo/ui`](/versions/latest/sdk/ui.md) removes that round-trip. Its `value` is an observable state object created with `useNativeState`, and `onChangeText` can be a worklet that runs synchronously on the UI thread. The formatted value lands in the same frame as the keystroke.
...
Native/UI thread:
1. Keystroke
→ 2. onChangeText worklet formats the string
→ 3. Worklet writes value and selection together
→ 4. Field shows the formatted text in the same frame
...
-   `@expo/ui`: The universal `TextInput` is available in SDK 56 and later. See the [installation instructions](/versions/latest/sdk/ui/universal/textinput.md#installation).
...
The platform-specific text fields from `@expo/ui` support the same controlled and worklet patterns. See the controlled text field examples for [Jetpack Compose](/versions/latest/sdk/ui/jetpack-compose/textfield.md#controlled-text-field) and [SwiftUI](/versions/latest/sdk/ui/swift-ui/textfield.md#controlled-text-field), and the [worklet masking example](/versions/latest/sdk/ui/universal/textinput.md#worklet-masking) for the universal `TextInput`.
...
-   [`Checkbox`](/versions/latest/sdk/checkbox.md) from `expo-checkbox` pairs `value` with `onValueChange`.
-   [`Picker`](/versions/latest/sdk/picker.md) from `@react-native-picker/picker` pairs `selectedValue` with `onValueChange`.
...
-   [`TextInput` in the `@expo/ui` reference](/versions/latest/sdk/ui/universal/textinput.md): The worklet-based input, its installation, and the full prop list.
-   [Keyboard handling](/guides/keyboard-handling.md): Managing the layout around the on-screen keyboard.
