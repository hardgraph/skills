...
Google One Tap validates tokens more strictly [#google-one-tap-validates-tokens-more-strictly]
...
OAuth2 token-request builders are now async [#oauth2-token-request-builders-are-now-async]
...
SCIM writes are safer [#scim-writes-are-safer]
...
SCIM connections are scoped to an organization [#scim-connections-are-scoped-to-an-organization]
...
SCIM gains durable group resources [#scim-gains-durable-group-resources]
...
Multi-host allowedHosts no longer trusts forwarded headers by default [#multi-host-allowedhosts-no-longer-trusts-forwarded-headers-by-default]
...
Drizzle schema uses singular relation keys with usePlural [#drizzle-schema-uses-singular-relation-keys-with-useplural]
...
Magic-link and email-OTP sign-in can clear unproven linked accounts [#magic-link-and-email-otp-sign-in-can-clear-unproven-linked-accounts]
...
Tracking prereleases [#tracking-prereleases]
