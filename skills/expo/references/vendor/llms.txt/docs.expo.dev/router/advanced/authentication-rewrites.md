...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/router/advanced/authentication-rewrites/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/router/advanced/authentication-rewrites/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Authentication (redirects)](https://docs.expo.dev/router/advanced/authentication-rewrites.md) (this page)
...
> SDK 53 introduced [Protected routes](/router/advanced/protected.md), a more powerful method of handling authentication. Please follow this guide if you are using SDK 52 and earlier.
...
Create a nested [layout route](/router/basics/navigation-layouts.md) that checks whether users are authenticated before rendering the child route components. This layout route redirects users to the sign-in screen if they are not authenticated.
...
You may encounter the following error when the app attempts to perform navigation without a navigator mounted in the [root layout](/router/basics/navigation-layouts.md#root-layout).
