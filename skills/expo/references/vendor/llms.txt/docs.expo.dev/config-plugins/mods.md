...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/config-plugins/mods/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/config-plugins/mods/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Mods](https://docs.expo.dev/config-plugins/mods.md) (this page)
...
| `mods.android.colors` | `withAndroidColors` ([Example](https://github.com/expo/expo/blob/main/packages/@expo/config-plugins/src/android/PrimaryColor.ts)) | - | Modify the **android/app/src/main/res/values/colors.xml** as JSON (parsed with [`xml2js`](https://www.npmjs.com/package/xml2js)). |
| `mods.android.colorsNight` | `withAndroidColorsNight` ([Example](https://github.com/expo/expo/blob/main/packages/expo-splash-screen/plugin/src/withAndroidSplashStyles.ts)) | - | Modify the **android/app/src/main/res/values-night/colors.xml** as JSON (parsed with [`xml2js`](https://www.npmjs.com/package/xml2js)). |
| `mods.android.styles` | `withAndroidStyles` ([Example](https://github.com/expo/expo/blob/main/packages/expo-splash-screen/plugin/src/withAndroidSplashStyles.ts)) | - | Modify the **android/app/src/main/res/values/styles.xml** as JSON (parsed with [`xml2js`](https://www.npmjs.com/package/xml2js)). |
...
**You don't use mods as top-level functions (for example `with.android.manifest`) directly in your config plugin.** When you need to use a mod, you use _mod plugins_ in your config plugins. These mod plugins are provided by the `expo/config-plugins` library and wrap top-level mod functions and behind the scenes they perform various tasks. To see a list of available mods, check out the [mod plugins provided by `expo/config-plugins`](/config-plugins/mods.md#available-mod-plugins).
...
For example, if you want to write a mod to update the Xcode Project's "product name", you'll create a config plugin file that uses the [`withXcodeProject`](/config-plugins/mods.md#ios) mod plugin.
...
> See [Create a module with a config plugin](/modules/config-plugin-and-native-module-tutorial.md) for a step-by-step guide on how to create a standalone package plugin.
