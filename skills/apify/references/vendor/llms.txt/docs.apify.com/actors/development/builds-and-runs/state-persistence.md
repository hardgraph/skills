...
url: https://docs.apify.com/actors/development/builds-and-runs/state-persistence.md
parents:
...
previous: [Runs](https://docs.apify.com/actors/development/builds-and-runs/runs.md)
next: [Permissions](https://docs.apify.com/actors/development/permissions.md)
---
...
Long-running [Actor](https://docs.apify.com/actors.md) jobs may need to migrate between servers. Without state persistence, your job's progress is lost during migration, causing it to restart from the beginning on the new server. This can be costly and time-consuming.
...
Once your Actor receives the `migrating` event, the Apify platform will shut it down and restart it on a new server within one minute. To speed this process up and ensure state consistency, you can manually reboot the Actor in the `migrating` event handler using the `Actor.reboot()` method available in the [Apify SDK for JavaScript](https://docs.apify.com/sdk/js/reference/class/Actor#reboot) or [Apify SDK for Python](https://docs.apify.com/sdk/python/reference/class/Actor#reboot).
