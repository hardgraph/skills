...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/router/advanced/apple-handoff/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/router/advanced/apple-handoff/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Apple Handoff](https://docs.expo.dev/router/advanced/apple-handoff.md) (this page)
...
-   Handoff requires [universal links](/linking/into-your-app.md) to be configured, at least on iOS, and contain the `activitycontinuation` object.
...
You can use the following command to generate the **apple-app-site-association** file based on your [app config](/versions/latest/config/app.md):
...
See [Test the deep link](/linking/into-your-app.md#test-the-deep-link) guide to test handoff in development.
...
-   "Receiving activities from Handoff in your app on other devices." is performed by an [App Delegate Subscriber](/modules/appdelegate-subscribers.md) in the Expo Head native module. It is used to redirect you to the correct route when you handoff to your native app.
