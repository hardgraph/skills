...
If you used `makeAssistantToolUI` or `useAssistantToolUI` for a backend, MCP, or LangGraph tool, the tool executes elsewhere. Prefer a `"use generative"` toolkit with `execute: externalTool()` so the compiler omits the server entry, while the client keeps your renderer as `type: "backend"`:
...
Register it like any toolkit: `config={config}` where `const config = AuiConfig({ tools: Tools({ toolkit }) })`. Render-only entries upload no schema and run no browser code — they only attach UI for matching tool-call message parts. For MCP server catalogs, spread `defineMcpToolkit({ ... })` in the same generative toolkit.
