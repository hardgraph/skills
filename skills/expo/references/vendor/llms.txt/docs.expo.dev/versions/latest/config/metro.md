...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/config/metro/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/config/metro/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [metro.config.js](https://docs.expo.dev/versions/latest/config/metro.md) (this page)
...
See more information about **metro.config.js** in the [customizing Metro guide](/guides/customizing-metro.md).

## Environment variables

Expo CLI can load environment variables from **.env** files. Learn more about how to use environment variables in Expo CLI in the [environment variables guide](/guides/environment-variables.md).

EAS CLI uses a different mechanism for environment variables, except when it invokes Expo CLI for compiling and bundling. Learn more about [environment variables in EAS](/eas/environment-variables.md).
...
Then, ensure [CSS is setup](/versions/latest/config/metro.md#css) in the **metro.config.js** file.
...
[Tailwind CSS](/guides/tailwind.md) — Learn how to configure and use Tailwind CSS in your Expo project.
...
Metro doesn't have a very expressive plugin system for transforming files, instead opt to use the [**babel.config.js**](/versions/latest/config/babel.md) and caller object to customize the transformation.
...
This feature is enabled by default and can be disabled by setting `experiments.onDemandFilesystem` to `false` in your [app config](/versions/latest/config/app.md):
...
> This guide is versioned and will need to be revisited when upgrading/downgrading Expo. Alternatively, use [Expo Prebuild](/more/glossary-of-terms.md#prebuild) for fully automated setup.

Projects that don't use [Expo Prebuild](/more/glossary-of-terms.md#prebuild) must configure native files to ensure the Expo Metro config is always used to bundle the project.
...
Development mode entry files can be enabled by using the [`expo-dev-client`](/versions/latest/sdk/dev-client.md) package. Alternatively you can add the following configuration:
