...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/guides/why-metro/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/guides/why-metro/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Why Metro?](https://docs.expo.dev/guides/why-metro.md) (this page)
...
-   React Native DevTools, including first-class support for [network and JS debugging](/debugging/tools.md#debugging-with-react-native-devtools), is exclusively available with Metro and Hermes.
...
The Expo team collaborates with Meta to develop Metro for Expo Router, adding features like [file-based routing](/develop/app-navigation.md), [web support](/guides/customizing-metro.md#web-support), [bundle splitting](/guides/customizing-metro.md#bundle-splitting), [tree shaking](/guides/tree-shaking.md), [CSS](/versions/latest/config/metro.md#css), [DOM components](/guides/dom-components.md), server components, and [API routes](/router/web/api-routes.md).
...
Expo leverages Metro's technology to create novel functionality like [DOM components](/guides/dom-components.md). This allows a React component in a native app to be dynamically bundled as an entire website with all the same defaults as the parent app, on-demand.
