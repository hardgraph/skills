...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/develop/user-interface/system-bars/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/develop/user-interface/system-bars/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [System bars](https://docs.expo.dev/develop/user-interface/system-bars.md) (this page)
...
[Safe areas](/develop/user-interface/safe-areas.md) — Learn how to add safe areas for screen components inside your Expo project.
...
The status bar appears at the top of the screen on both Android and iOS. You can customize it using [`expo-status-bar`](/versions/latest/sdk/status-bar.md). It provides a `StatusBar` component that you can use to control the appearance of the status bar while your app is running using the [`style`](/versions/latest/sdk/status-bar.md#style) property or the [`setStatusBarStyle`](/versions/latest/sdk/status-bar.md#statusbarsetstatusbarstylestyle-animated) method:
...
To control the `StatusBar` visibility, you can set the [`hidden`](/versions/latest/sdk/status-bar.md#hidden) property to `true` or use the [`setStatusBarHidden`](/versions/latest/sdk/status-bar.md#statusbarsetstatusbarhiddenhidden-animation) method.

### Navigation bar configuration (Android only)

On Android devices, the Navigation Bar appears at the bottom of the screen. You can customize it using the [`expo-navigation-bar`](/versions/latest/sdk/navigation-bar.md) library. It provides a `NavigationBar` component that you can use to set the style of the navigation bar using the [`setStyle`](/versions/latest/sdk/navigation-bar.md#navigationbarsetstylestyle) method:
...
To control the `NavigationBar` visibility, you can use the [`hidden`](/versions/latest/sdk/navigation-bar.md#hidden) prop or the [`NavigationBar.setHidden`](/versions/latest/sdk/navigation-bar.md#sethiddenhidden) method.
