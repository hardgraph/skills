...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/debugging/runtime-issues/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/debugging/runtime-issues/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Runtime issues](https://docs.expo.dev/debugging/runtime-issues.md) (this page)
- [Tools](https://docs.expo.dev/debugging/tools.md)
...
> Already familiar with React Native debugging? See [Debugging tools](/debugging/tools.md) for Expo-specific tooling like React Native DevTools and the built-in profiler.

## Development errors

They are common errors that you encounter while developing your app. Delving into them isn't always straightforward. Usually, debugging when running your app with [Expo CLI](/more/expo-cli.md) is enough.

One way you can debug these issues is by looking at the [stack trace](/debugging/errors-and-warnings.md#stack-traces). However, in some scenarios, looking at the stack trace isn't enough as the error message traced might be a little more cryptic. For such errors, follow the steps below:
...
**The best first step in addressing a production error is to reproduce it locally.** Once you reproduce an error locally, you can follow the [development debugging process](/debugging/runtime-issues.md#development-errors) to isolate and address the root cause.
...
-   **Use native log tools.** Connect a device that reproduces the crash and use [`adb logcat` or the Console app](/debugging/runtime-issues.md#viewing-native-logs) to capture the native log output. Native logs often reveal the root cause when JavaScript error boundaries don't catch the problem.
...
-   **Check your crash reporting dashboard.** If you use [Sentry](/guides/using-sentry.md), [BugSnag](/guides/using-bugsnag.md), or a similar service, check for the crash there first. These services provide stack traces, device info, and reproduction context.

### App crashes on certain (older) devices

This might indicate that there is a performance issue. You likely need to run your app through a profiler to get a better idea of what processes are killing the app, and [React Native provides some great documentation for this](https://reactnative.dev/docs/profiling). We also recommend using [React Native DevTools](/debugging/tools.md#debugging-with-react-native-devtools) and the included [profiler](/debugging/tools.md#profiling-javascript-performance), which makes it super easy to identify JavaScript performance sinks in your app.
...
With Expo, you can integrate a reporting service like [Sentry](/guides/using-sentry.md) or [BugSnag](/guides/using-bugsnag.md) to get more insights in real-time.
