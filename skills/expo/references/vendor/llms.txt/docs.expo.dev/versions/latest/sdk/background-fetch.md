...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/sdk/background-fetch/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/sdk/background-fetch/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
> **[Deprecated](/more/release-statuses.md#deprecated):** The `expo-background-fetch` library is being replaced by a new version in [`expo-background-task`](/versions/latest/sdk/background-task.md). `expo-background-fetch` is not receiving patches and will be removed in an upcoming release.

`expo-background-fetch` provides an API to perform [background fetch](https://developer.apple.com/documentation/uikit/core_app/managing_your_app_s_life_cycle/preparing_your_app_to_run_in_the_background/updating_your_app_with_background_app_refresh) tasks, allowing you to run specific code periodically in the background to update your app. This module uses [TaskManager](/versions/latest/sdk/task-manager.md) Native API under the hood.
...
On iOS the `BackgroundFetch` library requires you to use a [development build](/develop/development-builds/introduction.md) since Background Fetch is not enabled in the iOS Expo Go app.
...
If you're not using Continuous Native Generation ([CNG](/workflow/continuous-native-generation.md)) or you're using a native **ios** project manually, then you'll need to add the following to your **Expo.plist** file:
...
> **Deprecated:** Use [`getStatusAsync()`](/versions/latest/sdk/background-task.md#backgroundtaskgetstatusasync) from `expo-background-task` instead. The `expo-background-fetch` package has been deprecated.
...
> **Deprecated:** Use [`registerTaskAsync()`](/versions/latest/sdk/background-task.md#backgroundtaskregistertaskasynctaskname-options) from `expo-background-task` instead. The `expo-background-fetch` package has been deprecated.
...
| --- | --- | --- |
| `taskName` | `string` | Name of the task to register. The task needs to be defined first - see [`TaskManager.defineTask`](/versions/latest/sdk/task-manager.md#taskmanagerdefinetasktaskname-taskexecutor) for more details. |
...
> **Deprecated:** Use the [`registerTaskAsync()`](/versions/latest/sdk/background-task.md#backgroundtaskregistertaskasynctaskname-options) method from expo-background-task package, and specify [`BackgroundTaskOptions`](/versions/latest/sdk/background-task.md#backgroundtaskoptions) argument instead, when setting task interval time.
...
> **Deprecated:** Use [`unregisterTaskAsync()`](/versions/latest/sdk/background-task.md#backgroundtaskunregistertaskasynctaskname) from `expo-background-task` instead. The `expo-background-fetch` package has been deprecated.
