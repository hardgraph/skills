...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/eas-update/debug/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/eas-update/debug/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
You are here: EAS > EAS Update > Troubleshooting
Pages in this section:
- [Debugging](https://docs.expo.dev/eas-update/debug.md) (this page)
...
> If we are not using EAS Build, our Deployments page will be empty. Follow the guide on [debugging configuration without EAS Build](/eas-update/debug.md#configuration-without-eas-build) instead.
...
If the deployment channel is unexpected, it means our build was not built with the correct channel. To fix this, [configure our channel](/eas-update/debug.md#configure-channel) and rebuild our app.

### Unexpected runtime version

If the deployment runtime version is unexpected, it means our build was not built with the correct runtime version. To fix this, [configure our runtime version](/eas-update/debug.md#configure-runtime-version) and rebuild our app.

### Unexpected branch

If the deployment has an unexpected branch, we need to [map our channel to the correct branch](/eas-update/debug.md#map-channel-to-branch).

### Missing updates

The displayed deployment does not have any updates. To fix this, [publish an update to the branch](/eas-update/debug.md#publish-update). If an update was already published, check the [Updates page](https://expo.dev/accounts/%5Baccount%5D/projects/%5Bproject%5D/updates) to make sure it matches the runtime version of our build.

### Missing branch

The displayed deployment has the correct channel, but it is not linked to a branch. To fix this, [map the channel to the correct branch](/eas-update/debug.md#map-channel-to-branch).

### Missing deployment

If our deployment is not displayed, it means our build is not configured properly for EAS Update. To fix this, [configure our channel](/eas-update/debug.md#configure-channel), [configure our runtime version](/eas-update/debug.md#configure-runtime-version) and verify our [general configuration](/eas-update/debug.md#verifying-app-configuration). We'll need to rebuild our app after making these changes.
...
EAS Update is designed to automatically roll back to the previous update if it detects that a new update crashed shortly after launch. See [how EAS Update detects crashes and rolls back to a previous working version](/eas-update/error-recovery.md#explaining-the-error-recovery-flow) for more information.

To diagnose the error causing the update crash:

-   See the [Troubleshooting guide on runtime issues](/debugging/runtime-issues.md) to apply a strategy to identify the error.
-   After identifying the error, publish a new update that fixes the crash to resolve the issue.

A common reason a new update does not work but embedded code does is due to a missing environment variable. See [how environment variables work with EAS Update](/eas/environment-variables/usage.md#using-environment-variables-with-eas-update) for more information.
...
-   Verify which assets are downloaded by users and their sizes by inspecting the [asset list](/eas-update/debug.md#viewing-all-assets-included-in-an-update).
-   Reproduce the issue on your own device and examine the `expo-updates` [logs entries](/versions/latest/sdk/updates.md#updatesreadlogentriesasyncmaxage) surfaced from the native layer.
...
By default, it is `{ "policy": "appVersion" }`, but we can change our runtime to [use a different policy or a specific version](/eas-update/runtime-versions.md). Then, we can run a command like `eas build --profile preview` to create a build with the runtime version we expect.
...
Create a [development version of our build](/eas-update/expo-dev-client.md). It will help us preview published updates inside a problematic build.
...
-   Print the [Update.Constants](/versions/latest/sdk/updates.md#constants) to verify our configuration.
-   [Examine log entries](/versions/latest/sdk/updates.md#updatesreadlogentriesasyncmaxage) surfaced from the native layer.
-   Fetch and [load updates manually](/versions/latest/sdk/updates.md#example-check-for-updates-manually).

## Configuration issues

Our app is still not receiving the expected update despite following the [basic guide](/eas-update/debug.md).
...
Follow the [Building Locally guide](/eas-update/standalone-service.md) to configure our app's channel and runtime version. We'll also need to make sure our [general configuration](/eas-update/debug.md#expo-updates-configuration) is correct.
...
The `expo-updates` library exports a variety of functions to interact with updates once the app is already running. In certain cases, making a call to fetch an update and seeing an error message can help us narrow down the root cause. We can make a simulator build of the project and manually check to see if updates are available or if there are errors when fetching updates. See the code example to [check for updates manually](/versions/latest/sdk/updates.md#example-check-for-updates-manually).
...
Another way to identify the root cause of an issue is to look at the network requests that the app is making to EAS servers, then viewing the responses. We recommend using a program like [Proxyman](https://proxyman.com/) or [Charles Proxy](https://www.charlesproxy.com/) to watch network requests from our app.
...
We also provide a [step-by-step guide to try out EAS Update quickly](/eas-update/standalone-service.md) in a local development environment using Android Studio or Xcode, with either release or debug builds of the app.

#### Android local builds

-   Set the debug environment variable: `export EX_UPDATES_NATIVE_DEBUG=1`
-   [Ensure the desired channel is set in your **AndroidManifest.xml**](/eas-update/getting-started.md#configure-the-update-channel)
-   Execute a [debug build](/debugging/runtime-issues.md#native-debugging) of the app with Android Studio or from the command line.
...
-   [Ensure the desired channel is set in our **Expo.plist**](/eas-update/getting-started.md#configure-the-update-channel)
...
-   Execute a [debug build](/debugging/runtime-issues.md#native-debugging) of the app with Xcode or from the command line.
