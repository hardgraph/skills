...
Once you've created your application, it will be in the "Unverified" state. In the Unverified state, your application can be authorized by up to 100 distinct Convex teams, which lets you test and roll out your application before requesting verification. Once your application reaches 100 connected teams, no additional teams will be able to authorize it until it is verified. Verifying your application removes this limit. The number of teams that have authorized your application, and how close it is to the limit, is shown next to the application in your [Team Settings](https://dashboard.convex.dev/team/settings/applications/oauth-apps).
...
To have your application be verified and be accessible for an unlimited number of Convex teams, it must meet the following criteria:

* The application description has an explanation of the capabilities and planned future capabilities of your application.
* You provide a link to your live application. A working integration is required for verification.
...
![OAuth authorization page](/assets/images/oauth-page-00000000.png)
...
where `state` is the same value you provided earlier, and `code` is a randomly generated string like `00000000`. This code **is not** the final auth token - you'll need to exchange it for one within 10 minutes.
