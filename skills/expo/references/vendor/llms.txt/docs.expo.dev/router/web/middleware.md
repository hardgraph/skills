...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/router/web/middleware/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/router/web/middleware/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Server middleware](https://docs.expo.dev/router/web/middleware.md) (this page)
...
> Server middleware is in [alpha](/more/release-statuses.md#alpha) and is available in SDK 54 and later. It requires a [deployed server](/router/web/api-routes.md#deployment) for production use.

Server middleware in Expo Router allows you to run code before requests reach your routes, enabling powerful server-side functionality like authentication and logging for every request. Unlike [API routes](/router/web/api-routes.md) that handle specific endpoints, middleware runs for **every** request in your app, so it should run as quickly as possible to avoid slowing down your app's performance. Client-side navigation such as on native, or in a web app when using [`<Link />`](/versions/latest/sdk/router/link.md#link), will not move through the server middleware.

## Setup

### Enable server middleware in your app configuration

First, configure your app to use server output by adding the server configuration to your [app config](/versions/latest/config/app.md):
...
1.  The middleware function runs first with an [immutable request](/router/web/middleware.md#request-immutability).
...
-   Client-side navigation using [`<Link />`](/versions/latest/sdk/router/link.md#link) or [`router`](/versions/latest/sdk/router.md#imperativerouter)
...
-   Middleware runs exclusively on the server and only for HTTP requests. It does not execute during client-side navigation, for example, with [`<Link />`](/versions/latest/sdk/router/link.md#link) or native app screen transitions.
-   The request object passed to middleware is [immutable](/router/web/middleware.md#request-immutability) to prevent side effects. You cannot modify headers or consume the request body, ensuring it remains available for route handlers.
-   You can only have one root-level **+middleware.ts** in your app.
-   The same limitations that [apply to API routes](/router/web/api-routes.md#known-limitations) also apply to middleware.
