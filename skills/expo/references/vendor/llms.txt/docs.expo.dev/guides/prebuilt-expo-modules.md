...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/guides/prebuilt-expo-modules/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/guides/prebuilt-expo-modules/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Precompiled Expo Modules](https://docs.expo.dev/guides/prebuilt-expo-modules.md) (this page)
...
Set the `ios.usePrecompiledModules` property of the [`expo-build-properties`](/versions/latest/sdk/build-properties.md) config plugin to `false` to build every Expo Module from source. This applies to both local builds and EAS Build, and takes effect the next time you run `npx expo prebuild`:
...
For EAS Build, create an [EAS environment variable](/eas/environment-variables/manage.md):
