...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/sdk/auth-session/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/sdk/auth-session/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
`AuthSession` enables web browser-based authentication (for example, browser-based OAuth flows) in your app by utilizing [WebBrowser](/versions/latest/sdk/webbrowser.md) and [Crypto](/versions/latest/sdk/crypto.md). For implementation details, refer to this reference, and for usage, see the [Authentication](/guides/authentication.md) guide.

> **Note:** `AuthSession` enables general-purpose OAuth and OpenID Connect browser-based auth workflows. Where available, we recommend using a library supplied by your identity provider, as it will handle implementation details specific to that provider. For example, use [`@react-native-google-signin/google-signin`](/guides/google-authentication.md) for Google authentication and [`react-native-fbsdk-next`](/guides/facebook-authentication.md) for Facebook. For more information, see [Authentication](/develop/authentication.md) overview.
...
> In order to close the popup window on web, you need to invoke `WebBrowser.maybeCompleteAuthSession()`. See the [GitHub example](/guides/authentication.md#github) for more info.
...
| `options`(optional) | [Omit](https://www.typescriptlang.org/docs/handbook/utility-types.html#omittype-keys)<[CreateURLOptions](/versions/latest/sdk/linking.md#createurloptions), 'queryParams'\> |
...
| redirectUri | `string` | After completing an interaction with a resource owner the server will redirect to this URI. Learn more about [linking in Expo](/linking/overview.md). [Section 3.1.2](https://tools.ietf.org/html/rfc6749#section-3.1.2) |
...
Type: [Omit](https://www.typescriptlang.org/docs/handbook/utility-types.html#omittype-keys)<[AuthSessionOpenOptions](/versions/latest/sdk/webbrowser.md#authsessionopenoptions), 'windowFeatures'\> extended by:
...
| windowFeatures(optional) | [WebBrowserWindowFeatures](/versions/latest/sdk/webbrowser.md#webbrowserwindowfeatures) | Supported platforms: Web. Features to use with `window.open()`. |
...
| native(optional) | `string` | Manual scheme to use in existing React Native projects and production builds. Takes precedence over all other properties. You must define the URI scheme that will be used in a custom-built native application or production build. The value should conform to your native app's URI schemes. You can see conformance with `npx uri-scheme list`. |
...
> **Deprecated:** See [Facebook authentication](/guides/facebook-authentication.md).
...
| Property | Type | Description |
| --- | --- | --- |
| androidClientId(optional) | `string` | Android native client ID for use in development builds and existing React Native projects. |
| iosClientId(optional) | `string` | iOS native client ID for use in development builds and existing React Native projects. |
...
> **Deprecated:** See [Google authentication](/guides/google-authentication.md).
...
| Property | Type | Description |
| --- | --- | --- |
| androidClientId(optional) | `string` | Android native client ID for use in production builds and existing React Native projects. |
| iosClientId(optional) | `string` | iOS native client ID for use in production builds, existing React Native projects, and development builds. |
