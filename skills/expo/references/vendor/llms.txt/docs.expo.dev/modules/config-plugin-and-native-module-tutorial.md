...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/modules/config-plugin-and-native-module-tutorial/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/modules/config-plugin-and-native-module-tutorial/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Create a module with a config plugin](https://docs.expo.dev/modules/config-plugin-and-native-module-tutorial.md) (this page)
...
[Config plugins](/config-plugins/introduction.md) let you customize native Android and iOS projects generated with `npx expo prebuild` in [Continuous Native Generation (CNG)](/workflow/continuous-native-generation.md) projects. You can use them to add properties to native config files, copy assets to native projects, or apply advanced configurations, such as adding an [app extension target](/build-reference/app-extensions.md).

As an app developer, config plugins help you apply customizations not exposed in the default [app config](/workflow/configuration.md). As a library author, they enable you to configure native projects automatically for developers using your library.
...
[Plugins](/config-plugins/introduction.md#plugin-function) are synchronous functions that accept an `ExpoConfig` and return a modified `ExpoConfig`. By convention, these functions are prefixed with the word `with`. Name your plugin `withMyApiKey` or use a different name, as long as it follows this convention.
...
To inject your custom API keys into **AndroidManifest.xml** and **Info.plist**, use helper [`mods` provided by `expo/config-plugins`](/config-plugins/mods.md). These make it easy to modify native files. For this example, use `withAndroidManifest` and `withInfoPlist`.
...
[Expo Modules API Reference](/modules/module-api.md) — A reference to create native modules using Kotlin and Swift.

[Additional platform support](/modules/additional-platform-support.md) — Learn how to add support for macOS and tvOS platforms.
