...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/guides/server-components/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/guides/server-components/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [React Server Components](https://docs.expo.dev/guides/server-components.md) (this page)
...
> [Experimentally](/more/release-statuses.md#experimental) available. This is a [beta](/more/release-statuses.md#beta) release and subject to breaking changes.
...
Expo Router enables support for [React Server Components](https://react.dev/reference/rsc/server-components) on all platforms. This is an early [preview](/more/release-statuses.md#preview) of a feature that will be enabled by default in Expo Router.
...
Library authors can test their modules support Server Components by using `jest-expo`. Learn more in the [Testing React Server Components](/guides/testing-rsc.md) guide.
...
> This mode is [experimental](/more/release-statuses.md#experimental).
...
> Universal React Server Components are still in [beta](/more/release-statuses.md#beta).
...
[Deploy native servers to EAS](/router/web/api-routes.md#native-deployment) — Deploy and link versioned servers to your production native apps.
...
-   [`generateStaticParams`](/router/web/static-rendering.md#generatestaticparams) is only partially supported in full React Server Components mode.
