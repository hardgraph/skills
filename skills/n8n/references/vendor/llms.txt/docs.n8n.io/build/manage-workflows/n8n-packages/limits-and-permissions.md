> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/build/manage-workflows/n8n-packages/limits-and-permissions.md).
...
Reference for what constrains [package](/build/manage-workflows/n8n-packages.md) export and import: size caps, license features, API key scopes, and observability.
...
{% hint style="warning" %}
The upload itself is capped separately, by [`N8N_PAYLOAD_SIZE_MAX`](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables/endpoints.md), which defaults to 16 MiB and isn't specific to packages. That cap applies to the compressed file, so it's usually the one you hit first: a package only reaches 300 MiB uncompressed if it compresses to under 16 MiB.
...
Export caps IDs at 300 per group, counted separately for workflow, folder, and project IDs. See [Choose what to export](/build/manage-workflows/n8n-packages/export-a-package.md#choose-what-to-export).
...
* `packageFormatVersion` has to match exactly. See [Version compatibility](/build/manage-workflows/n8n-packages/package-format.md#version-compatibility).
...
Missing permission to create a tag or a data table is reported as a blocking issue rather than a `403`. See [Why an import is refused](/build/manage-workflows/n8n-packages/how-import-works.md#why-an-import-is-refused).
...
For what each event's payload holds, see [Events](/build/manage-workflows/n8n-packages/export-a-package.md#events) on the export page and [Events](/build/manage-workflows/n8n-packages/import-a-package.md#events) on the import page.

## Read next

* [Export a package](/build/manage-workflows/n8n-packages/export-a-package.md) and [Import a package](/build/manage-workflows/n8n-packages/import-a-package.md) for the options these limits apply to.
* [How import works](/build/manage-workflows/n8n-packages/how-import-works.md) for how the license and scope checks fit into an import.
