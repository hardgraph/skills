> This page location: APIs & SDKs > SDKs > Neon Management SDK
...
- [Migrate from @neondatabase/api-client](https://neon.com/docs/reference/migrate-api-client-to-sdk)
- [Neon CLI](https://neon.com/docs/cli)

**Source code**

- [@neon/sdk on npm](https://www.npmjs.com/package/@neon/sdk)
...
It replaces [`@neondatabase/api-client`](https://www.npmjs.com/package/@neondatabase/api-client), the deprecated Axios-based SDK. New projects should use `@neon/sdk`. See the [migration guide](https://neon.com/docs/reference/migrate-api-client-to-sdk) for method mapping and error-handling changes.

**Note: Not every endpoint has an ergonomic wrapper**

`createNeonClient` namespaces cover common workflows (projects, branches, Postgres resources, snapshots, and more). They do **not** wrap every Platform API operation. For endpoints without a namespace method, use the [`raw` layer](https://neon.com/docs/reference/typescript-sdk#raw-layer) below or the [Neon API Reference](https://neon.com/docs/reference/api).
...
In the reference tables, the **Returns** column names the resolved resource, the type of `data` on success (or the value returned directly when `throwOnError` is set). A method resolving to `void` has no resource body; [`Paginated`](https://neon.com/docs/reference/typescript-sdk#lazy-auto-paginated-lists)`<T>` is the lazy, auto-paginated list described below. Every method also accepts an optional trailing options argument (`{ throwOnError?, waitForReadiness?, signal? }`), omitted from the tables for brevity.
...
Methods labeled Paginated return a [`Paginated`](https://neon.com/docs/reference/typescript-sdk#lazy-auto-paginated-lists)`<T>`; the cursor is managed for you:
...
For raw API calls that return an `operations` array, use [`neon.operations.waitFor`](https://neon.com/docs/reference/typescript-sdk#neonoperations) instead.

## Namespaces

The client groups the API into resource namespaces. Projects and branches are the core surfaces: [`projects`](https://neon.com/docs/reference/typescript-sdk#neonprojects) create, manage, and share projects, and [`branches`](https://neon.com/docs/reference/typescript-sdk#neonbranches) branch a project's data and schema. The Postgres data plane lives under [`postgres`](https://neon.com/docs/reference/typescript-sdk#neonpostgres): compute endpoints, roles, databases, the Data API, and connection strings.

Branch-scoped platform services include [`storage`](https://neon.com/docs/reference/typescript-sdk#neonstorage) (S3-compatible object storage), [`functions`](https://neon.com/docs/reference/typescript-sdk#neonfunctions), [`credentials`](https://neon.com/docs/reference/typescript-sdk#neoncredentials), [`aiGateway`](https://neon.com/docs/reference/typescript-sdk#neonaigateway), and [`auth`](https://neon.com/docs/reference/typescript-sdk#neonauth) (Managed Better Auth, OAuth providers, and users). For data lifecycle and async work, use [`snapshots`](https://neon.com/docs/reference/typescript-sdk#neonsnapshots) for point-in-time snapshots and restore, and [`operations`](https://neon.com/docs/reference/typescript-sdk#neonoperations) to poll asynchronous operations.

Account-level surfaces round out the client: [`consumption`](https://neon.com/docs/reference/typescript-sdk#neonconsumption) for billing metrics, [`apiKeys`](https://neon.com/docs/reference/typescript-sdk#neonapikeys), and [`regions` / `user`](https://neon.com/docs/reference/typescript-sdk#neonregions--user).

## neon.projects

Create, manage, and share Neon projects. One API call per method; `list` is paginated. REST: [Projects API](https://neon.com/docs/reference/api/projects)

| Method                            | Returns                                                                                                    | Arguments                                                                                                               |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `list(query?)`                    | [`Paginated`](https://neon.com/docs/reference/typescript-sdk#lazy-auto-paginated-lists)`<ProjectListItem>` | `query`: `{ search?, org_id?, limit? }`                                                                                 |
| `get(id)`                         | `Project`                                                                                                  |                                                                                                                         |
...
Branch a project's data and schema; optionally attach compute in one workflow. REST: [Branches API](https://neon.com/docs/reference/api/branches)

| Method                                         | Returns                                                                                           | Arguments                                                                                                              |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `list(projectId, query?)`                      | [`Paginated`](https://neon.com/docs/reference/typescript-sdk#lazy-auto-paginated-lists)`<Branch>` | `query`: `{ search?, sort_by?, sort_order?, include_deleted? }`                                                        |
...
The Postgres data plane of a branch: compute endpoints, roles, databases, the Data API, and a connection-string helper. REST: [Endpoints](https://neon.com/docs/reference/api/endpoints), [Branches](https://neon.com/docs/reference/api/branches), [Data API](https://neon.com/docs/reference/api/dataapi)
...
Branch-scoped, S3-compatible object storage. `get` returns whether storage is enabled and the branch's S3 endpoint metadata; buckets and objects are nested underneath. REST: [Storage](https://neon.com/docs/reference/api/storage), [Buckets](https://neon.com/docs/reference/api/buckets)
...
Branch-scoped Neon Functions. REST: [Functions API](https://neon.com/docs/reference/api/functions)

| Method                                      | Returns                                                                                                 | Arguments                                                                                                                                                |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list(projectId, branchId, query?)`         | [`Paginated`](https://neon.com/docs/reference/typescript-sdk#lazy-auto-paginated-lists)`<NeonFunction>` | `query`: `{ limit? }`                                                                                                                                    |
...
Branch-scoped credentials with explicit scopes. Secrets (`api_token`, `s3_secret_access_key`) are returned once, on `create`. REST: [Credentials API](https://neon.com/docs/reference/api/credentials)
...
Branch-scoped AI Gateway endpoint metadata. REST: [AI Gateway API](https://neon.com/docs/reference/api/ai-gateway)
...
Point-in-time snapshots, restore, and backup schedules. REST: [Snapshots API](https://neon.com/docs/reference/api/snapshots)
...
Read operations and wait for them to finish. REST: [Operations API](https://neon.com/docs/reference/api/operations)

| Method                          | Returns                                                                                              | Arguments                                             |
| ------------------------------- | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `list(projectId)`               | [`Paginated`](https://neon.com/docs/reference/typescript-sdk#lazy-auto-paginated-lists)`<Operation>` |                                                       |
...
Branch-scoped Managed Better Auth. The legacy project-scoped endpoints are deprecated and remain raw-only. REST: [Authentication API](https://neon.com/docs/reference/api/auth)
...
Cursor-paginated billing metrics. Each method takes `{ from, to, granularity, org_id, project_ids? }`, where `from`/`to` are ISO timestamps, `granularity` is `"hourly"` | `"daily"` | `"monthly"`, and `org_id` names the org to report on; `perBranchV2` also requires `project_ids`. Consumption requires a Scale plan or above. REST: [Consumption API](https://neon.com/docs/reference/api/consumption)

| Method                | Returns                                                                                                                   |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `perProject(query)`   | [`Paginated`](https://neon.com/docs/reference/typescript-sdk#lazy-auto-paginated-lists)`<ConsumptionHistoryPerProject>`   |
| `perProjectV2(query)` | [`Paginated`](https://neon.com/docs/reference/typescript-sdk#lazy-auto-paginated-lists)`<ConsumptionHistoryPerProjectV2>` |
| `perBranchV2(query)`  | [`Paginated`](https://neon.com/docs/reference/typescript-sdk#lazy-auto-paginated-lists)`<ConsumptionHistoryPerBranchV2>`  |
...
Manage account-level API keys. REST: [API Keys API](https://neon.com/docs/reference/api/api-keys)
...
Active regions and the current account. REST: [Regions](https://neon.com/docs/reference/api/regions), [Users](https://neon.com/docs/reference/api/users)
