> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/changelog/v10-migration-guide.md).
...
Your existing workflows will use the legacy order, while new workflows will execute using the v1 order. You can configure the execution order for each workflow in [workflow settings](/build/manage-workflows/configure-workflow-settings.md).
...
### EXECUTIONS\_PROCESS and "own" mode <a href="#executionsprocess-and-own-mode" id="executionsprocess-and-own-mode"></a>
...
This change makes [User Management](/administer/manage-users-and-access.md) mandatory and removes support for other authentication methods, such as BasicAuth and External JWT. Note that the number of permitted users on [n8n.cloud](https://n8n.cloud/) or custom plans still varies depending on your subscription.
...
For more information about date transformation functions, please refer to the [official documentation](/build/work-with-data/transform-data/expression-reference.md).
...
### Removed N8N\_USE\_DEPRECATED\_REQUEST\_LIB <a href="#removed-n8nusedeprecatedrequestlib" id="removed-n8nusedeprecatedrequestlib"></a>
...
If you build custom nodes, refer to [HTTP request helpers](/connect/create-nodes/build-your-node/reference/http-request-helpers.md) for more information on migrating to the new interface.

[PR #6413](https://github.com/n8n-io/n8n/pull/6413)

### Removed WEBHOOK\_TUNNEL\_URL <a href="#removed-webhooktunnelurl" id="removed-webhooktunnelurl"></a>

As of version 0.227.0, n8n has renamed the `WEBHOOK_TUNNEL_URL` configuration option to `WEBHOOK_URL`. In n8n 1.0, `WEBHOOK_TUNNEL_URL` has been removed. Update your setup to reflect the new name. For more information about this configuration option, refer to [the docs](/deploy/host-n8n/configure-n8n/basic-configuration/configuration-examples/configure-webhook-urls-with-reverse-proxy.md).
