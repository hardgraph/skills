...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/sdk/camera/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/sdk/camera/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
The [`with-camera`](https://github.com/expo/examples/tree/master/with-camera) example comes with `expo-camera` already installed and configured:
...
To localize the iOS camera or microphone permission message, add `NSCameraUsageDescription` or `NSMicrophoneUsageDescription` to the `ios` object in each [locale file](/guides/localization.md#translating-app-metadata). Keep `cameraPermission` and `microphonePermission` as the default values. Expo writes the localized values to **InfoPlist.strings** during prebuild.
...
> On native platforms, the local image URI is temporary. Use [`FileSystem.copy`](/versions/latest/sdk/filesystem.md#copydestination-options-1) to make a permanent copy of the image.
...
Type: Class extends [NativeModule](/versions/v57.0.0/sdk/expo.md#nativemoduletype)<[CameraEvents](#cameraevents)\>
...
This package automatically adds the `CAMERA` permission to your app. If you want to record videos with audio, you have to include the `RECORD_AUDIO` in your **app.json** inside the [`expo.android.permissions`](/versions/latest/config/app.md#permissions) array.
