...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/router/migrate/from-expo-webpack/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/router/migrate/from-expo-webpack/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Expo Webpack](https://docs.expo.dev/router/migrate/from-expo-webpack.md) (this page)
...
> `@expo/webpack-config` is [deprecated](/more/release-statuses.md#deprecated) and not receiving any new feature updates.

Expo Router supports [static rendering on web](/router/web/static-rendering.md), which enables search engine optimization (SEO), social media previews, and faster loading times, unlike Expo Webpack. Along with the benefits of React Navigation, it enables automatic deep linking, [type safety](/router/reference/typed-routes.md), [deferred bundling](/router/web/async-routes.md), [modular HTML templates](/router/web/static-rendering.md#root-html), [static rendering on web](/router/web/static-rendering.md), and more.
...
| `tsconfig.json` paths | ✓ | ✗ |
| Tree Shaking | ([Partial support](/guides/tree-shaking.md)) | ✓ |
...
-   **Recommended**: `web.output: "static"` which outputs a new HTML file for each route in the app. This approach lets you [dynamically generate the entire HTML template using the **src/app/+html.tsx** file](/router/web/static-rendering.md#root-html).
...
Like before, the root [**babel.config.js**](/versions/latest/config/babel.md) file is used for both web and native. You can change the preset by using the `platform` property in the API caller:
...
The [`expo-constants`](/versions/latest/sdk/constants.md) library can be used to access the **app.json** in-app. Behind the scenes, this is accomplished by setting `process.env.APP_MANIFEST` with the stringified contents of the **app.json** file.
...
See [hosting with sub-paths](/more/expo-cli.md#hosting-with-sub-paths) for more information.
...
If you were using custom bundler plugins, see [Expo Metro config](/versions/latest/config/metro.md) for adding custom functionality to your bundler pipeline.

## Navigation

If you used React Navigation for navigating between screens in `@expo/webpack-config`, see the [migration guide for React Navigation](/router/migrate/from-react-navigation.md).

## Deployment

Check out [Publishing websites](/guides/publishing-websites.md) on how to deploy Expo Router websites to various hosting providers.
