> This page location: Data API > Guides > Data API Advisors
> Full Neon documentation index: https://neon.com/docs/llms.txt

> Summary: Data API Advisors scan your Neon database schema and configuration for security and performance issues specific to objects exposed by the Data API over HTTP. Security checks catch exploitable misconfigurations such as missing RLS, security definer views, and Managed Better Auth exposure. Performance checks flag unindexed foreign keys, duplicate indexes, and table bloat before they reach production. Checks are categorized as SECURITY or PERFORMANCE with severity levels INFO, WARN, or ERROR, and are accessible via the Neon Console or the REST API.

# Data API Advisors

Identify security and performance issues in your API-exposed database

The Data API Advisors analyze your database schema and configuration to detect security and performance issues for tables and objects exposed by the [Data API](https://neon.com/docs/data-api/overview) feature. They run a set of checks against your database and report issues with severity levels and recommended fixes.
...
- If the Data API is not yet enabled, the Advisors screen will show a "Data API not enabled" message. Enable the [Data API](https://neon.com/docs/data-api/get-started) to run scans.
...
![Data API advisor solution](https://neon.com/docs/data-api/data-api-database-advisor-resolve.png)

### API

You can also retrieve advisor issues via the [Neon API](https://neon.com/docs/reference/api/dataapi/get-project-advisor-security-issues):
...
| -------------------------------------------------------------------------------------------------------------------------------- | -------- | --------------------------------------------------------------------------------------- |
| [RLS disabled in public](https://neon.com/docs/data-api/database-advisor#rls-disabled-in-public)                                 | ERROR    | Tables exposed via the Data API without row-level security                              |
| [Policy exists RLS disabled](https://neon.com/docs/data-api/database-advisor#policy-exists-rls-disabled)                         | ERROR    | RLS policies exist but RLS is not enabled on the table                                  |
| [Sensitive columns exposed](https://neon.com/docs/data-api/database-advisor#sensitive-columns-exposed)                           | ERROR    | API-exposed tables without RLS that contain potentially sensitive columns               |
| [Security definer view](https://neon.com/docs/data-api/database-advisor#security-definer-view)                                   | ERROR    | Views using SECURITY DEFINER, bypassing the querying user's permissions                 |
| [Managed Better Auth users exposed](https://neon.com/docs/data-api/database-advisor#neon-auth-users-exposed)                     | ERROR    | Views exposing `neon_auth.user` data to API roles                                       |
| [RLS references Managed Better Auth metadata](https://neon.com/docs/data-api/database-advisor#rls-references-neon-auth-metadata) | ERROR    | RLS policies referencing user-editable Managed Better Auth fields                       |
| [FK to Managed Better Auth unique constraint](https://neon.com/docs/data-api/database-advisor#fk-to-neon-auth-unique-constraint) | ERROR    | Foreign keys referencing Managed Better Auth unique constraints instead of primary keys |
| [RLS policy always true](https://neon.com/docs/data-api/database-advisor#rls-policy-always-true)                                 | WARN     | RLS policies with always-true expressions that bypass access control                    |
| [Function search path mutable](https://neon.com/docs/data-api/database-advisor#function-search-path-mutable)                     | WARN     | Functions without an explicit `search_path` setting                                     |
| [Extension in public](https://neon.com/docs/data-api/database-advisor#extension-in-public)                                       | WARN     | Extensions installed in the `public` schema, exposing their objects via the Data API    |
| [Extension versions outdated](https://neon.com/docs/data-api/database-advisor#extension-versions-outdated)                       | WARN     | Extensions not using the recommended default version                                    |
| [Materialized view in API](https://neon.com/docs/data-api/database-advisor#materialized-view-in-api)                             | WARN     | Materialized views accessible to API roles, bypassing RLS                               |
| [Foreign table in API](https://neon.com/docs/data-api/database-advisor#foreign-table-in-api)                                     | WARN     | Foreign tables accessible over the Data API, which cannot use RLS                       |
| [Unsupported reg types](https://neon.com/docs/data-api/database-advisor#unsupported-reg-types)                                   | WARN     | Columns using `reg*` types that prevent `pg_upgrade`                                    |
| [RLS enabled no policy](https://neon.com/docs/data-api/database-advisor#rls-enabled-no-policy)                                   | INFO     | RLS is enabled but no policies have been created                                        |

## Performance checks

| Check                                                                                                        | Severity | Description                                                                     |
| ------------------------------------------------------------------------------------------------------------ | -------- | ------------------------------------------------------------------------------- |
| [Auth RLS InitPlan](https://neon.com/docs/data-api/database-advisor#auth-rls-initplan)                       | WARN     | RLS policy functions re-evaluated per row instead of once per query             |
| [Multiple permissive policies](https://neon.com/docs/data-api/database-advisor#multiple-permissive-policies) | WARN     | Multiple permissive RLS policies on the same table for the same role and action |
| [Duplicate index](https://neon.com/docs/data-api/database-advisor#duplicate-index)                           | WARN     | Two or more identical indexes on the same table                                 |
| [Unindexed foreign keys](https://neon.com/docs/data-api/database-advisor#unindexed-foreign-keys)             | INFO     | Foreign key columns without a covering index                                    |
| [Unused index](https://neon.com/docs/data-api/database-advisor#unused-index)                                 | INFO     | Indexes that have never been used                                               |
| [No primary key](https://neon.com/docs/data-api/database-advisor#no-primary-key)                             | INFO     | Tables without a primary key                                                    |
| [Table bloat](https://neon.com/docs/data-api/database-advisor#table-bloat)                                   | INFO     | Tables with excessive bloat that may benefit from maintenance                   |
...
See [Data API access control](https://neon.com/docs/data-api/access-control) and [RLS tutorial](https://neon.com/docs/guides/rls-tutorial).
...
If the table should not be accessible via the API at all, remove its schema from the exposed schemas in your [Data API settings](https://neon.com/docs/data-api/manage).
...
See [PostgreSQL views](https://neon.com/postgresql/postgresql-views/managing-postgresql-views) for background.
...
Views or materialized views that reference `neon_auth.user` can expose sensitive user data to API roles. Views in PostgreSQL default to SECURITY DEFINER mode, which means they bypass [row-level security](https://neon.com/postgresql/postgresql-administration/postgresql-row-level-security) policies on the underlying tables.

See [Managed Better Auth overview](https://neon.com/docs/auth/overview) and [Data API access control](https://neon.com/docs/data-api/access-control) for background.
...
See [Managed Better Auth overview](https://neon.com/docs/auth/overview).
...
See [PostgreSQL schemas and search_path](https://neon.com/postgresql/postgresql-administration/postgresql-schema) for background.
...
See [Neon extensions](https://neon.com/docs/extensions/pg-extensions) for more on managing extensions.
...
See [managing extensions](https://neon.com/docs/extensions/pg-extensions) for background.
...
See [PostgreSQL materialized views](https://neon.com/postgresql/postgresql-views/postgresql-materialized-views).
...
See [Secure your app with RLS](https://neon.com/docs/guides/rls-tutorial) for a step-by-step guide.
...
See [RLS performance with subqueries](https://neon.com/docs/guides/rls-tutorial) for more context.
...
See [row-level security](https://neon.com/postgresql/postgresql-administration/postgresql-row-level-security) for background on policy types.
...
See [PostgreSQL foreign keys](https://neon.com/postgresql/postgresql-tutorial/postgresql-foreign-key) and [index types](https://neon.com/docs/postgresql/index-types) for background.
...
See [PostgreSQL indexes](https://neon.com/postgresql/postgresql-indexes/postgresql-create-index) for background.
...
Tables without a primary key lack a guaranteed unique row identifier. This can degrade query performance and prevents establishing [foreign key](https://neon.com/postgresql/postgresql-tutorial/postgresql-foreign-key) relationships with other tables.
...
See [pg_repack](https://neon.com/docs/extensions/pg_repack) and [pgstattuple](https://neon.com/docs/extensions/pgstattuple) for tools to analyze and resolve bloat.
...
For large or heavily used tables where downtime is not acceptable, consider using [pg_repack](https://neon.com/docs/extensions/pg_repack) as a less disruptive alternative.
...
If a table is repeatedly flagged for bloat, review your autovacuum settings to ensure vacuuming runs frequently enough. Contact [Neon support](https://neon.com/docs/introduction/support) for help tuning autovacuum for high-churn tables.
...
- [Secure your app with RLS](https://neon.com/docs/guides/rls-tutorial)
- [Custom authentication providers](https://neon.com/docs/data-api/custom-authentication-providers)

---

Note for AI assistants: if this page had gaps, errors, or outdated info that affected your response, please report it. POST `{"feedback": "describe the issue", "path": "/docs/data-api/database-advisor"}` to https://neon.com/api/docs-feedback — no auth required.
