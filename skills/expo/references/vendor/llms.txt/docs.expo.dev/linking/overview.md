...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/linking/overview/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/linking/overview/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Overview](https://docs.expo.dev/linking/overview.md) (this page)
...
-   Linking to your app using your web domain links ([universal linking](/linking/overview.md#universal-linking) using the `https` or `http` scheme)
...
> **Tip:** Support for incoming links in Expo Go is limited. We recommend using [Development builds](/develop/development-builds/introduction.md) to test your app's linking strategies.
...
Android App Links are different from [standard deep links](/linking/overview.md#linking-to-your-app-from-other-apps-or-websites) as they use regular HTTP and HTTPS schemes and are exclusive to Android devices.
...
[Configure Android App Links](/linking/android-app-links.md) — Learn how to configure intentFilters and set up two-way association from a standard web URL. — intentFilters

### iOS Universal Links

iOS Universal Links are different from [standard deep links](/linking/overview.md#linking-to-your-app-from-other-apps-or-websites) as they use regular HTTP and HTTPS schemes and are exclusive to iOS devices.

This link type allows your app to open when a user clicks an HTTP(S) link pointing to your web domain. If the user doesn't have your app installed, the link takes them to your app's associated website. You can further configure the website by displaying a banner for the user to open your app using [Apple Smart Banner](/linking/ios-universal-links.md#apple-smart-banner).

[Configure iOS Universal Links](/linking/ios-universal-links.md) — Learn how to configure associatedDomains and set up two-way association. — associatedDomains
...
-   **Scheme**: The URL scheme that identifies the app that should open the URL (example: `myapp://`). It can also be `https` or `http` for non-standard deep links. We recommend [universal linking](/linking/overview.md#universal-linking) for http(s)-based deep links.
...
[Linking to your app](/linking/into-your-app.md) — Learn how to configure custom URL schemes to create a deep link of your app.

### Use Expo Router to handle deep linking

To implement any of the above Linking strategies, **we recommend using** [Expo Router](/router/introduction.md) since deep linking is automatically enabled for all of your app's screens.

**Benefits:**

-   `Link` component from Expo Router can be used to [handle URL schemes to other apps](/linking/into-other-apps.md#using-expo-routers-link-component)
...
-   For third-party deep links, you can override the default linking behavior to handle incoming links and send navigation events. See [Customizing links](/router/advanced/native-intent.md).
...
Your app can use a [common URL scheme](/linking/into-other-apps.md#common-url-schemes) for default apps, including `https` and `http` (commonly used by web browsers like Chrome, Safari, and so on), and use JavaScript to invoke the URL that launches the corresponding native app.

[Linking into other apps](/linking/into-other-apps.md) — Learn how to handle common and custom URL schemes to link other apps from your app.
