...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/more/expo-cli/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/more/expo-cli/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Expo CLI](https://docs.expo.dev/more/expo-cli.md) (this page)
...
-   [Start a server](/more/expo-cli.md#develop) for developing your app: `npx expo start`.
-   [Generate the native Android and iOS directories](/more/expo-cli.md#prebuild) for your project: `npx expo prebuild`.
-   [Build and run](/more/expo-cli.md#compiling) the native apps locally: `npx expo run:ios` and `npx expo run:android`.
-   [Install and update packages](/more/expo-cli.md#install) that work with the version of `react-native` in your project: `npx expo install package-name`.
...
> Projects that are not using [Expo Prebuild](/more/expo-cli.md#prebuild) will need to perform additional setup to ensure all custom Expo bundling features work: [Metro setup for existing React Native apps](/versions/latest/config/metro.md#existing-react-native-apps).
...
| J | Open React Native DevTools for any connected device that is using Hermes as the JavaScript engine. See [debugging JavaScript with Hermes](/guides/using-hermes.md#javascript-debugger). |
| O | Open project code in your editor. This can be configured with the `EXPO_EDITOR` and `EDITOR` [environment variables](/more/expo-cli.md#environment-variables). |
...
-   `--port`: Port to start the dev server on (does not apply to webpack or [tunnel URLs](/more/expo-cli.md#tunneling)). Use `--port 0` to automatically use the first available port. Default: **8081**.
...
The dev server exposes `/_expo/open` so external tools, such as cloud agents, remote preview services, CI scripts, can introspect the deep links the CLI would use, and optionally trigger the same action as pressing I / A / W in the **Terminal UI**. It supplements the legacy `/_expo/link` endpoint, which returns a `307` redirect to a deep link scheme that non-mobile clients can't follow.

| Method | Effect |
| --- | --- |
| `GET` | Dry run: returns the deep link as JSON. Safe to call across tunnels. |
| `POST` | Opens the project locally — equivalent to pressing I / A / W in the **Terminal UI**. Restricted to same-origin requests. |
...
-   `runtime`: choose how the URL is resolved.
    -   `default` (omitted): mirrors what pressing I / A does. It picks the dev client when the server was started with `--dev-client`, falls back to a disambiguation page when the project has both Expo Go and a development build, otherwise opens Expo Go.
...
The endpoint reflects mid-run state changes — pressing S to toggle between Expo Go and the dev client, or installing `expo-dev-client` while the server is running, both show up on the next request.
...
A React Native app consists of two parts: a native runtime ([compiling](/more/expo-cli.md#compiling)), and static files like JavaScript bundles and assets ([exporting](/more/expo-cli.md#exporting)). Expo CLI provides commands for performing both tasks.
...
Building locally is useful for developing native modules and [debugging complex native issues](/debugging/runtime-issues.md#native-debugging). Building remotely with `eas build` is a much more resilient option due to the pre-configured cloud environment.
...
For example, if your project does not have an **ios** directory in the root of your project, then `npx expo run:ios` will first run `npx expo prebuild -p ios` before compiling your app. For more information on this process, see [Expo Prebuild](/workflow/continuous-native-generation.md).
...
-   `-p, --port <port>`: Port to start the development server. **Default: 8081**. This is only relevant for development builds. Production builds will [export](/more/expo-cli.md#exporting) the project and embed the files in the native binary before installing them on a device.
...
-   In EAS Build, use a matching Gradle command like [`:app:assembleDebugOptimized` in **eas.json**](/build-reference/apk.md#configuring-a-profile-to-build-apks)
...
This build is not automatically code-signed for submission to the Google Play Store. This command should be used to test bugs that may only show up in production builds. To generate a production build that is code signed for the Play Store, we recommend using [EAS Build](/build/introduction.md).
...
For more information, see the [Local builds using Android product flavors](/guides/local-app-development.md#local-builds-using-android-product-flavors) guide.
...
-   `--platform <platform>`: Choose the platform to compile for: 'ios', 'android', 'all'. **Default: all**. 'web' is also available if configured in the app config. For more information, see [Customizing Metro](/guides/customizing-metro.md).
...
-   `--no-ssg`: Skip exporting static HTML files for web routes. This option only generates server code inside the **dist** directory. Useful for [API routes](/router/web/api-routes.md).

#### Hosting with sub-paths

> [Experimental](/more/release-statuses.md#experimental) functionality.

You can configure the prefix for static assets by setting the `experiments.baseUrl` field in your [app config](/workflow/configuration.md):
...
> **[Deprecated](/more/release-statuses.md#deprecated)**: In SDK 50 and later, Expo Webpack has been deprecated in favor of universal Metro (`npx expo export`). Learn more in [migrating from Webpack to Expo Router](/router/migrate/from-expo-webpack.md).
...
Native source code must be generated before a native app can compile. Expo CLI provides a unique and powerful system called _prebuild_, that generates the native code for your project. To learn more, read the [Expo Prebuild docs](/workflow/continuous-native-generation.md).
...
[Using ESLint](/guides/using-eslint.md) — Learn more about ensuring best practices with ESLint in an Expo project.
...
-   `-t, --type`: [Type of config](/more/expo-cli.md#config-type) to show.
...
-   `prebuild`: The config that is used for [Expo Prebuild](/workflow/continuous-native-generation.md) including async modifiers. This is the only time the config is not serializable.
-   `introspect`: A subset of the `prebuild` config that only shows in-memory modifications like `Info.plist` or **AndroidManifest.xml** changes. Learn more about [introspection](/config-plugins/development-and-debugging.md#introspection).
...
| `EXPO_NO_TYPESCRIPT_SETUP` | **boolean** | Prevents the CLI from forcing TypeScript to be configured on `npx expo start`.For more information, see [TypeScript guide](/guides/typescript.md). |
...
| `EXPO_NO_TELEMETRY` | **boolean** | Disables anonymous usage collection. [Learn more about telemetry](/more/expo-cli.md#telemetry). |
...
| `EXPO_PUBLIC_FOLDER` | **string** | Public directory path to use with Metro for web. [Learn more about customizing Metro](/guides/customizing-metro.md).Default: `public` |
...
| `EXPO_UNSTABLE_TREE_SHAKING` | **boolean** | , Experimental, SDK 52+Enable unstable tree shaking support across all platforms. See [tree shaking](/guides/tree-shaking.md) for more details. |
