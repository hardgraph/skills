...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/router/advanced/stack-toolbar/" "<actionable feedback>"
...
#### Label primitive is not supported on Android

On Android, `Stack.Toolbar.Button` renders only its icon and badge — the `Stack.Toolbar.Label` children are dropped. If you need label-like UI on Android, embed a custom component using [`Stack.Toolbar.View`](/versions/latest/sdk/router/stack.md#stacktoolbarview).
