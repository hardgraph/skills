> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/changelog/readme.md).
...
* [Release notes](/changelog/release-notes.md): a listing of all feature-level updates in each release.
...
Old-style release notes pages for [2.x](/changelog/release-notes-2.x.md), [1.x](/changelog/release-notes-1.x.md), and [0.x](/changelog/release-notes-0.x.md) remain archived. Everything in the 2.x archive is covered by this changelog and the Release notes.
{% endhint %}
...
Refer to [Large webhook responses](/deploy/host-n8n/configure-n8n/scaling/enable-queue-mode.md#large-webhook-responses) for the full configuration, upgrade order, and troubleshooting.

***

## App-only authentication for Microsoft nodes

**Released:** 2026-07-07 in [n8n 2.30](/changelog/release-notes.md#n8n230)

You can now authenticate Microsoft nodes with a [Microsoft Entra Service Principal](/integrations/builtin/credentials/microsoftentraserviceprincipal.md), so workflows run as an application instead of a signed-in user. OneDrive and Outlook gained the option in 2.29; Excel 365, Microsoft Teams, and Microsoft To Do follow in 2.30, all sharing a single app-only credential.
...
*OneDrive and Outlook support released in 2.29 (2026-06-30).*

### mTLS authentication for Kafka

The [Kafka credential](/integrations/builtin/credentials/kafka.md) now supports mutual TLS: provide a CA certificate, client certificate, and private key (PEM) to connect to brokers that require client-certificate authentication. mTLS applies to the Kafka node, the Kafka Trigger, and the credential test, and n8n validates that certificate and key match before you save.

***

## AI Assistant: describe a goal, get a working automation

**Released:** 2026-07-09 in [n8n 2.29.9](/changelog/release-notes.md#n8n229)
...
<figure><img src="/files/togQUjNIhXg7n33C0OEC" alt="The AI Assistant entry point: a chat box asking what to automate, with suggestions like Score my leads."><figcaption><p>Describe what you want to automate, or start from a suggestion.</p></figcaption></figure>
...
Learn more in the [AI Assistant documentation](/build/ways-of-building-workflows/ai-assistant.md).
...
**Released:** 2026-06-30 in [n8n 2.29](/changelog/release-notes.md#n8n229)
...
Learn more in the [documentation](/administer/observe-and-log/track-usage-with-insights.md#disable-or-configure-insights-metrics-collection).
...
**Released:** 2026-06-29 in [n8n 2.28](/changelog/release-notes.md#n8n228)
...
<figure><img src="/files/hqP8UQFoaXz17BsrbEzF" alt="Three connected nodes selected on the canvas, with the Group nodes action and its Ctrl/Cmd + G shortcut shown above the selection."><figcaption><p>Select a connected run of nodes, then group them with the Group nodes button or Ctrl/Cmd + G.</p></figcaption></figure>
...
Learn more in the [Canvas Groups documentation](/build/understand-workflows/workflow-components/canvas-groups.md).

### GitHub node: manage the full pull request lifecycle

The [GitHub node](/integrations/builtin/app-nodes/n8n-nodes-base.github.md#operations) now has a dedicated Pull Request resource. Create pull requests (including drafts and cross-fork PRs), update, close, and reopen them, read and add comments, fetch diffs and patches, and merge with merge, squash, or rebase. These native operations replace the custom HTTP Request setups that such tasks used to need. Errors are surfaced exactly as GitHub returns them, so failures are easy to diagnose.

### Webhook node: Only Run If

The [Webhook node](/integrations/builtin/core-nodes/n8n-nodes-base.webhook.md) gains an expression-based **Only run if** option that rejects requests that don't match a condition before an execution starts. Filter out health checks, retries, or irrelevant events at the door instead of starting a run that immediately exits: fewer no-op executions, less noise in your execution list, and saved execution quota.

***

## Move workflows between instances as packages

**Released:** 2026-06-16 in [n8n 2.27](/changelog/release-notes.md#n8n227)
...
Learn more in the [n8n Packages documentation](/build/manage-workflows/n8n-packages.md).
...
Learn more in the [OpenTelemetry tracing documentation](/deploy/host-n8n/keep-n8n-running/trace-executions-with-opentelemetry.md).

***

## Web search for AI agents

**Released:** 2026-06-02 in [n8n 2.25](/changelog/release-notes.md#n8n2251)
...
Learn more in the [documentation](/deploy/host-n8n/keep-n8n-running/trace-executions-with-opentelemetry.md#custom-span-attributes).
...
**Released:** 2026-05-12 in [n8n 2.21](/changelog/release-notes.md#n8n221)
...
**Released:** 2026-04-21 in [n8n 2.18](/changelog/release-notes.md#n8n218)
...
The [Slack Trigger](/integrations/builtin/trigger-nodes/n8n-nodes-base.slacktrigger.md) now offers **app\_home\_opened** as a dedicated event option. Previously, reacting to App Home opens meant subscribing to Any Event and filtering downstream, which started an execution for every unrelated Slack event.

### Linear Trigger: webhook signature verification

Linear credentials gain an optional signing secret. When set, the [Linear Trigger](/integrations/builtin/trigger-nodes/n8n-nodes-base.lineartrigger.md) verifies each incoming webhook's HMAC-SHA256 signature and validates its timestamp within a 60-second window, rejecting invalid or replayed requests with a 401.

***

## New model providers: Moonshot Kimi and Alibaba Cloud Model Studio

**Released:** 2026-04-13 in [n8n 2.17](/changelog/release-notes.md#n8n217)

Two model providers join n8n's AI lineup natively. **Moonshot Kimi** arrives as both a [chat-model sub-node](/integrations/builtin/cluster-nodes/sub-nodes/n8n-nodes-langchain.lmchatmoonshot.md) for AI Agents (with a dynamic model list, defaulting to kimi-k2.5) and a [standalone node](/integrations/builtin/app-nodes/n8n-nodes-langchain.moonshot.md) with multi-turn chat, tool calling, built-in web search, thinking mode, JSON responses, and image analysis. [**Alibaba Cloud Model Studio**](/integrations/builtin/app-nodes/n8n-nodes-langchain.alibabacloud.md) brings the Qwen family: chat with web search and agent-tool support, vision-language image analysis, text-to-image, and text- and image-to-video generation with automatic download of results.

More providers followed in later releases:

### MiniMax

*Released in 2.18 (2026-04-21).*

A [MiniMax chat-model sub-node](/integrations/builtin/cluster-nodes/sub-nodes/n8n-nodes-langchain.lmchatminimax.md) (OpenAI-compatible API, default MiniMax-M2.7, with a Hide Thinking option that strips reasoning traces for clean responses) plus a [standalone MiniMax node](/integrations/builtin/app-nodes/n8n-nodes-langchain.minimax.md) covering chat, image generation, asynchronous video generation, and text-to-speech with voice, emotion, speed, and pitch controls.

### NVIDIA Nemotron embeddings

*Released in 2.26 (2026-06-09).*
...
**Released:** 2026-04-07 in [n8n 2.16](/changelog/release-notes.md#n8n216)
...
Redaction is configured per workflow under **Workflow settings**, and reveal access is granted via project or instance settings to specific users only. See the [execution data redaction docs](/deploy/host-n8n/configure-n8n/security/redact-execution-data.md).
...
**Released:** 2026-03-30 in [n8n 2.15](/changelog/release-notes.md#n8n215)
...
**Released:** 2026-03-16 in [n8n 2.13](/changelog/release-notes.md#n8n213)
...
* [Custom roles](/administer/manage-users-and-access/set-permissions-and-roles-rbac/create-custom-roles.md) now include five secrets scopes: list, read, create, update, and delete.
...
Refer to [External secrets](/administer/manage-credentials/use-external-secret-stores.md) for more information.
...
{% hint style="info" %}
**Availability:** Enterprise. Requires [Environments](/administer/use-source-control-and-environments/set-up-source-control.md) configured.
{% endhint %}
...
**Released:** 2026-03-09 in [n8n 2.12](/changelog/release-notes.md#n8n212)

n8n now supports 1Password Connect Server as an [external secrets](/administer/manage-credentials/use-external-secret-stores.md) provider, alongside HashiCorp Vault, AWS Secrets Manager, Azure Key Vault, and GCP Secret Manager.
...
**Released:** 2026-03-02 in [n8n 2.11](/changelog/release-notes.md#n8n211)
...
<figure><img src="/files/Xh2ItSd6fy71UbxFdBd8" alt="Setting up Slack credentials with managed OAuth"><figcaption><p>Setting up Slack credentials with managed OAuth</p></figcaption></figure>
...
Instance admins now have a dedicated **Assignments** tab on each [custom role](/administer/manage-users-and-access/set-permissions-and-roles-rbac/create-custom-roles.md) showing every user assigned to that role, which project they're in, and a direct link to manage them — no more navigating project by project.

### Project-scoped external secrets: instance admin setup

Instance admins can now create vault connections scoped to a specific project. Secrets from that connection appear only within that project's credentials, not across the instance. Instance-level connections are unaffected. Refer to [External secrets](/administer/manage-credentials/use-external-secret-stores.md) for more information.

### Workflow execute as a separate permission scope

`workflow:execute` is now a distinct scope in [custom project roles](/administer/manage-users-and-access/set-permissions-and-roles-rbac/create-custom-roles.md), separate from editing and publishing. Users can be granted run access without being able to modify the workflow, which is a common compliance requirement for sensitive workflows.
...
**Released:** 2026-02-09 – 2026-02-13 in [n8n 2.8.0–2.8.3](/changelog/release-notes.md#n8n28)
...
This release builds on recent updates to the permissions model, including [custom project roles](/administer/manage-users-and-access/set-permissions-and-roles-rbac/create-custom-roles.md), to better support policy-driven governance.

<figure><img src="/files/3HivOcFAvSmZMgaAb3Zc" alt="The new Security &#x26; policies settings section"><figcaption><p>The new Security &#x26; policies settings section</p></figcaption></figure>
...
<figure><img src="/files/JTqdxl2jnWEfk1ui9NxA" alt="System roles and custom roles are now displayed in separate sections"><figcaption><p>System roles and custom roles are now displayed in separate sections</p></figcaption></figure>
...
Workflow publishing permissions for [custom roles](/administer/manage-users-and-access/set-permissions-and-roles-rbac/create-custom-roles.md) have been split into two separate scopes: `workflow:publish` and `workflow:unpublish`. This enables more precise access control in governance scenarios where unpublishing needs to be managed independently.
...
**Released:** 2026-01-26 in [n8n 2.6](/changelog/release-notes.md#n8n26)
...
Get precise control over where human judgment is required, without limiting what your agent can do. Learn more in the [human-in-the-loop tools docs](/build/integrate-ai/ai-examples/human-in-the-loop-for-tools.md).
...
**Released:** 2026-01-20 in [n8n 2.5](/changelog/release-notes.md#n8n25)
...
Learn more in the [Chat node documentation](/integrations/builtin/core-nodes/n8n-nodes-langchain.chat.md#operation).
...
**Released:** 2026-01-12 in [n8n 2.4](/changelog/release-notes.md#n8n24)
...
**Released:** 2025-12-22 in [n8n 2.2](/changelog/release-notes.md#n8n22)
...
<figure><img src="/files/V3qufQBMeHFvbNU0wFdz" alt="Custom Project Roles"><figcaption><p>Custom Project Roles</p></figcaption></figure>
...
**Released:** 2025-12-16 in [n8n 2.1](/changelog/release-notes.md#n8n21)
...
<figure><img src="/files/8sopBMwDysCOXgxjqM8r" alt="Time saved node in a workflow"><figcaption><p>Time saved node in a workflow</p></figcaption></figure>
...
<figure><img src="/files/xEp5ScspoCNJvC3p8GX8" alt=""><figcaption><p>Total time saved calculation</p></figcaption></figure>
