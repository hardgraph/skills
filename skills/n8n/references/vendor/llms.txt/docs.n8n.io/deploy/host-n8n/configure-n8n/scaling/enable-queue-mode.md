> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/deploy/host-n8n/configure-n8n/scaling/enable-queue-mode.md).
...
n8n doesn't support queue mode with binary data storage in filesystem. If your workflows need to persist binary data in queue mode, you can use [S3 external storage](/deploy/host-n8n/configure-n8n/scaling/use-external-storage.md).
{% endhint %}
...
!["Diagram showing the flow of data between the main n8n instance, Redis, the n8n workers, and the n8n database"](/files/HV5k1wPEAkfFIhiHi1Ul)
...
If your workers share a writable filesystem, give each worker process a unique event log path. Refer to [Per-process event log files](/administer/observe-and-log/stream-logs-to-external-systems.md#per-process-event-log-files) for details.
{% endhint %}

### Set encryption key <a href="#set-encryption-key" id="set-encryption-key"></a>

n8n automatically generates an encryption key upon first startup. You can also provide your own custom key using [environment variable](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables.md) if desired.
...
Set the encryption key for each worker node in a [configuration file](/deploy/host-n8n/configure-n8n/basic-configuration.md) or by setting the corresponding environment variable:
...
{% hint style="info" %}
**Database considerations** Refer to [Supported PostgreSQL versions](/deploy/host-n8n/configure-n8n/choose-n8ns-database.md#supported-postgresql-versions) for n8n's supported PostgreSQL versions.
...
Alternatively, you can set `executions.mode` to `queue` in the [configuration file](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables.md).
...
* [credentials overwrite endpoint](/administer/manage-credentials/credential-overwrites.md)
* [`/metrics`](/deploy/host-n8n/configure-n8n/basic-configuration/configuration-examples/enable-prometheus-metrics.md)
...
If you want to migrate data from one database to another, you can use the Export and Import commands. Refer to the [CLI commands for n8n](/deploy/host-n8n/configure-n8n/use-the-command-line.md#export-workflows-and-credentials) documentation to learn how to use these commands.
{% endhint %}
...
Set `N8N_WEBHOOK_RESPONSE_RELAY_OFFLOAD_ENABLED=true` on your workers to store a response body above the limit in [binary data storage](/deploy/host-n8n/configure-n8n/basic-configuration/use-environment-variables/binary-data.md) instead of failing the node. The queue message then carries a reference, the main instance streams the body from storage to the client, and n8n deletes the stored body once it delivers the response.
