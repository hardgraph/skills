> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/connect/connect-to-n8n-mcp-server.md).
...
2. Toggle **Enable MCP access** (requires instance owner or admin permissions).
   ...
   ![enable-mcp-access.png](/files/rwjUU10eQq6HuQpWtdPO)
...
4. *Connection details* button that shows detailed instructions for connecting MCP clients
   ...
   ![mcp\_page\_content.png](/files/eAVemhpDF59LlKJcnDVF)
...
* **Access Token**
  ...
  ![mcp\_connect\_menu.png](/files/KloN3oVzxhUgsSVnp52C)
...
3. Use the action menu in each client's row to revoke access for specific clients.
   ...
   ![mcp\_revoke\_client\_access.png](/files/t6NwlQJds2hd7NHdtw0p)
...
5. Update all connected MCP clients with the new value.
   ...
   ![mcp\_rotate\_token.png](/files/DJImb3Y2ipRcehjrv9fl)
...
You can use the **Options** menu <img src="/files/t9bqrsqJFxTTNwcjbZiO" alt="Options menu" data-size="line"> in the workflow list to toggle MCP access for all workflows in the current project or folder:

1. Navigate to the desired project and select **Workflows** from the top menu, then open a subfolder if required.
2. Select the **Options** menu <img src="/files/t9bqrsqJFxTTNwcjbZiO" alt="Options icon" data-size="line"> next to the name of the project or folder.
3. Select **Manage MCP access**, then either **Enable MCP** or **Disable MCP**.

![mcp\_bulk\_toggle.png](/files/htt9zqEDAMaXMmyTnIFj)
...
2. Option 2: From the workflow editor
   ...
   ![mcp\_workflow\_description.png](/files/HTxhDyB4YH17wLvwWa8Y)
