...
sourceCodeUrl: 'https://github.com/expo/expo/tree/sdk-57/packages/expo-apple-authentication'
...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/sdk/apple-authentication/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/sdk/apple-authentication/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
To enable the **Sign In with Apple** capability in your app, set the [`ios.usesAppleSignIn`](/versions/latest/config/app.md#usesapplesignin) property to `true` in your project's app config:
...
Apps that don't use [EAS Build](/build/introduction.md) must [manually configure](/build-reference/ios-capabilities.md#manual-setup) the **Apple Sign In** capability for their bundle identifier.

If you enable the **Apple Sign In** capability through the [Apple Developer Console](/build-reference/ios-capabilities.md#apple-developer-console), then be sure to add the following entitlements in your **ios/[app]/[app].entitlements** file:
...
| ERR_INVALID_SCOPE | An invalid [`AppleAuthenticationScope`](/versions/latest/sdk/apple-authentication.md#appleauthenticationscope) was passed in. |
