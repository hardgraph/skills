...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/eas-update/code-signing/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/eas-update/code-signing/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Code signing](https://docs.expo.dev/eas-update/code-signing.md) (this page)
...
> EAS Update Code Signing is only available to accounts subscribed to the EAS Production or Enterprise plans. See [EAS pricing plans](https://expo.dev/pricing).
...
-   The certificate validity duration is a setting that may vary based on the security needs of your app.
    ...
    -   A shorter validity duration will require [key rotation](/eas-update/code-signing.md#key-rotation) more frequently but is considered better practice since a compromised private key will have a sooner expiration which limits exposure.
...
The process of removing code signing from an app is similar to [key rotation](/eas-update/code-signing.md#key-rotation) and can be thought of as a key rotation to a `null` key.
