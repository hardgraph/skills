> This page location: APIs & SDKs > CLI > Projects and branches > diff
...
The `+++` branch comes from `--branch`, or the branch pinned in your [`.neon` context file](https://neon.com/docs/cli/set-context), or the project's default branch, in that order. `diff` covers every database on that branch unless you pass `--database`. Use `--output json` or `--output yaml` for a machine-readable diff, one entry per database.

`neon diff` is a shortcut for comparing branch schemas. To compare against a historical point in time (by timestamp or LSN), use the [`neon branches schema-diff`](https://neon.com/docs/cli/branches#schema-diff) subcommand.
