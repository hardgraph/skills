...
Inside a [Python Worker](https://developers.cloudflare.com/workers/languages/python/), pass a Worker binding instead of REST credentials. The binding path is asynchronous, so use `ainvoke`.
...
`self.env.MY_SEARCH` is a dedicated `ai_search` binding, not the Workers AI binding (`self.env.AI`). For a [namespace binding](https://developers.cloudflare.com/ai-search/concepts/namespaces/), pass `self.env.<NAMESPACE>.get("my-instance")`.
...
### [Python SDK quickstart](https://developers.cloudflare.com/ai-search/get-started/python/)

Create and manage AI Search instances from Python.

### [Retrieval configuration](https://developers.cloudflare.com/ai-search/configuration/retrieval/)
