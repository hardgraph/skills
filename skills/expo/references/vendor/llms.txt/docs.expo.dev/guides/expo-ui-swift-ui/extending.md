...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/guides/expo-ui-swift-ui/extending/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/guides/expo-ui-swift-ui/extending/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Extending with SwiftUI](https://docs.expo.dev/guides/expo-ui-swift-ui/extending.md) (this page)
...
Install `@expo/ui` in your project. See [Building SwiftUI apps with Expo UI](/guides/expo-ui-swift-ui.md) for more information.
...
##### Familiarity with Expo Modules API and SwiftUI

Basic familiarity with [Expo Modules API](/modules/overview.md) and [SwiftUI](https://developer.apple.com/swiftui/) is recommended.
...
1.  **Props class**: Extends `UIBaseViewProps` from `ExpoUI` to get automatic support for the [`modifiers`](/versions/latest/sdk/ui/swift-ui/modifiers.md) prop
...
-   Use the [built-in SwiftUI components](/versions/latest/sdk/ui/swift-ui.md) that come with Expo UI
