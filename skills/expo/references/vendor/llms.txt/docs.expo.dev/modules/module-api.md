...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/modules/module-api/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/modules/module-api/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Module API](https://docs.expo.dev/modules/module-api.md) (this page)
...
As you might have noticed in the snippets on the [Get Started](/modules/get-started.md) page, each module class must implement the `definition` function. The module definition consists of the DSL components that describe the module's functionality and behavior.
...
> **[Deprecated](/more/release-statuses.md#deprecated):** Use [`Constant`](/modules/module-api.md#constant) instead.
...
See the [Argument types](/modules/module-api.md#argument-types) section for more details on what types can be used in the function body.
...
Enables the module to be used as a native view. Definition components that are accepted as part of the view definition: [`Prop`](/modules/module-api.md#prop), [`Events`](/modules/module-api.md#events), [`GroupView`](/modules/module-api.md#groupview) and [`AsyncFunction`](/modules/module-api.md#asyncfunction).

[`AsyncFunction`](/modules/module-api.md#asyncfunction) in the view definition is added to the React ref of the React component representing the native view. Such async functions automatically receive an instance of the native view as the first argument and run on the UI thread by default.

#### Arguments

-   **viewType** — The class of the native view that will be rendered. Note: On Android, the provided class must inherit from the [`ExpoView`](/modules/module-api.md#expoview), on iOS it's optional. See [`Extending ExpoView`](/modules/module-api.md#extending-expoview).
...
> **Note:** This component can be used inside of the [`View`](/modules/module-api.md#view) block to define callback names. See [`View callbacks`](/modules/module-api.md#view-callbacks)
...
See [Sending events](/modules/module-api.md#sending-events) to learn how to send events from the native code to JavaScript/TypeScript.
...
> **Note:** This function is not available on Android — you may want to use [`OnActivityEntersForeground`](/modules/module-api.md#onactivityentersforeground) instead.
...
> **Note:** This function is not available on Android — you may want to use [`OnActivityEntersBackground`](/modules/module-api.md#onactivityentersbackground) instead.
...
> **Note:** This function is not available on iOS — you may want to use [`OnAppEntersForeground`](/modules/module-api.md#onappentersforeground) instead.
...
> **Note:** This function is not available on iOS — you may want to use [`OnAppEntersBackground`](/modules/module-api.md#onappentersbackground) instead.
...
The view definition consists of the DSL components that describe the view's functionality and behavior. Those components can only be used within a [`View`](/modules/module-api.md#view) closure.
...
This property can only be used within a [`View`](/modules/module-api.md#view) closure.
...
Similarly to the [`AsyncFunction`](/modules/module-api.md#asyncfunction) inside the module definition, you can define functions attached to the view ref to allow direct modification of the native view.
...
Enables the view to be used as a view group. Definition components that are accepted as part of the group view definition: [`AddChildView`](/modules/module-api.md#addchildview), [`GetChildCount`](/modules/module-api.md#getchildcount), [`GetChildViewAt`](/modules/module-api.md#getchildviewat), [`RemoveChildView`](/modules/module-api.md#removechildview), [`RemoveChildViewAt`](/modules/module-api.md#removechildviewat).
...
This property can only be used within a [`GroupView`](/modules/module-api.md#groupview) closure.
...
The built-in Convertibles are documented [further below](/modules/module-api.md#built-in-convertibles).
...
> **This feature is [experimental](/more/release-statuses.md#experimental).**
...
To export your view using the [`View`](/modules/module-api.md#view) component, your custom class must inherit from the `ExpoView`. By doing that you will get access to the [`AppContext`](/modules/module-api.md#appcontext) object. It's the only way of communicating with other modules and the JavaScript runtime. Also, you can't change constructor parameters, because provided view will be initialized by `expo-modules-core`.
...
To do this, in the module definition, you need to provide the event names that the module can send using the [Events](/modules/module-api.md#events) definition component. After that, you can use the `sendEvent(eventName, payload)` function on the module instance to send the actual event with some payload. For example, a minimal clipboard implementation that sends native events may look like this:
...
To subscribe to these events in JavaScript/TypeScript, use [`addListener`](/versions/latest/sdk/expo.md#addlistenereventname-listener) on the module object returned by `requireNativeModule`. Modules are extending the built-in [`EventEmitter`](/versions/latest/sdk/expo.md#eventemittertype) class. Alternatively, you can use [`useEvent`](/versions/latest/sdk/expo.md#useeventeventemitter-eventname-initialvalue) or [`useEventListener`](/versions/latest/sdk/expo.md#useeventlistenereventemitter-eventname-listener) hooks.
...
Some events are connected to a certain view. For example, the touch event should be sent only to the underlying JavaScript view which was pressed. In that case, you can't use `sendEvent` described in [`Sending events`](/modules/module-api.md#sending-events). The `expo-modules-core` introduces a view callbacks mechanism to handle view-bound events.

To use it, in the view definition, you need to provide the event names that the view can send using the [Events](/modules/module-api.md#events) definition component. After that, you need to declare a property of type `EventDispatcher` in your view class. The name of the declared property has to be the same as the name exported in the `Events` component. Later, you can call it as a function and pass a payload of type `[String: Any?]` on iOS and `Map<String, Any?>` on Android.
