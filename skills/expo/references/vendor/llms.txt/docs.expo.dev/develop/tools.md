...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/develop/tools/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/develop/tools/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Tools for development](https://docs.expo.dev/develop/tools.md) (this page)
...
| `npx expo prebuild` | Generates native Android and iOS directories using [Prebuild](/workflow/continuous-native-generation.md). |
...
| `npx expo lint` | [Setup and configures](/guides/using-eslint.md) ESLint. If ESLint is already configured, this command will [lint your project files](/guides/using-eslint.md#usage). |

In a nutshell, Expo CLI allows you to develop, compile, start your app, and more. See [Expo CLI reference](/more/expo-cli.md) for more available options and actions you can perform with the CLI.
...
This command performs checks and analyzes your project's codebase for common issues in [app config](/workflow/configuration.md) and **package.json** files, dependency compatibility, configuration files, and the overall health of the project. Once the check is complete, Expo Doctor outputs the results.
...
You can also use it to debug your app using VS Code's built-in debugger to set breakpoints, inspect variables, execute code through the debug console, and more. See [Debugging with VS Code](/debugging/tools.md#debugging-with-vs-code) for how to use this extension for debugging.
...
-   Click [this link](/get-started/set-up-your-environment.md?mode=expo-go) to go to Set up your environment guide
...
> **Note:** Expo Go is limited and not useful for building production-grade projects. Use [development builds](/get-started/set-up-your-environment.md?mode=development-build) instead.
...
> The `expo-go` CLI works for Android device, Android Emulator, iOS Simulator. Due to Apple's policies, an iPhone device does not support side-loading older app versions in general and the `expo-go` CLI does not support it.
...
To fix this, upgrading your project to a [supported SDK version](/versions/latest.md#each-expo-sdk-version-depends-on-a-react-native-version) is recommended. If you want to learn how to do it, see [Upgrade the project to a new SDK Version](/develop/tools.md#how-do-i-upgrade-my-project-from).
...
See [Upgrading Expo SDK guide](/workflow/upgrading-expo-sdk-walkthrough.md) for instructions for upgrading to a specific SDK version.
...
[Use libraries](/workflow/using-libraries.md) — See this guide to learn more about the difference between React Native core libraries, Expo SDK libraries, and third-party libraries. It also explains how to determine third-party library compatibility.
