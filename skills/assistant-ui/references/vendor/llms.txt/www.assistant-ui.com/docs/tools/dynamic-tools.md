...
> [!info]
>
> Keep the override keys stable after mount, and let only one mounted provider define a given tool name at a time. The null-returning overrides component re-binds the executor whenever the setter changes, without remounting the provider.
