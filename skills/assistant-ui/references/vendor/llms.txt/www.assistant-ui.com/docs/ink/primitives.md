...
Composable loading UI for active runs. `LoadingPrimitive.Root` only renders while the current thread is running, so it pairs naturally with status rows in terminal chat layouts.
...
This renders a compact status line such as `[=== ] Analyzing codebase... (3s)` while the assistant is streaming.

### Root

Runtime-aware `Box` container that renders children only while `s.thread.isRunning` is true.
...
Animated loader indicator. Supports the existing `ink-spinner` path plus a few built-in text-based variants for more terminal-native styles.
...
Elapsed time indicator for the current run. Uses message timing metadata when available and falls back to a local timer so it still works during manual testing.
