...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/guides/tree-shaking/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/guides/tree-shaking/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Tree shaking](https://docs.expo.dev/guides/tree-shaking.md) (this page)
...
Tree shaking (also referred to as _dead code removal_) is a technique to remove unused code from the production bundle. Expo CLI employs different techniques, including [minification](/guides/minify.md), to improve startup time by removing unused code.
...
The unreachable conditions are removed during [minification](/guides/minify.md):
...
> [Experimentally](/more/release-statuses.md#experimental) available in SDK 52 and later.
...
If the star export pulls in ambiguous exports such as `module.exports.ArrowUp` or `exports.ArrowDown`, then the optimization pass will not expand the star export and no exports will be removed from the barrel file. You can use [Expo Atlas](/guides/analyzing-bundles.md#analyzing-bundle-size-with-expo-atlas) to inspect the expanded exports.
