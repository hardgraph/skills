...
![Backups Page](/assets/images/backups-00000000.png)
...
Alternatively, you can generate an export in the same format with the [command line](/cli/reference/export.md):
...
Exported ZIP files that include [file storage](/file-storage/overview.md) will contain storage data in a `_storage` folder, with metadata like IDs and checksums in `_storage/documents.jsonl` and each file as `_storage/<id>`.
...
### How do backups work on dedicated hardware like D1024 and D2048?[​](#how-do-backups-work-on-dedicated-hardware-like-d1024-and-d2048 "Direct link to How do backups work on dedicated hardware like D1024 and D2048?")

In addition to the downloadable zip backups, dedicated deployments (D1024 and D2048) have physical backups configured automatically for every 24 hours and accessible for up to 7 days. Physical backups are faster and cheaper due to the ability to leverage the dedicated database hardware, however restores require contacting Convex support. Physical backups cannot be downloaded. Cross-deployment restores are not supported into dedicated deployments. Physical backups are included in the price of dedicated hardware.
