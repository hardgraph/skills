...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/router/advanced/stack/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/router/advanced/stack/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Stack](https://docs.expo.dev/router/advanced/stack.md) (this page)
...
> Screen composition API is in [alpha](/more/release-statuses.md#alpha) and available in SDK 55 and later.
...
[Stack Toolbar](/router/advanced/stack-toolbar.md) — Configure iOS header toolbar with support for Liquid Glass.
...
For example, the `index` route in the following layout structure shows a list of different user profiles in the app. Let's make the `[details]` route a [dynamic route](/router/basics/notation.md#square-brackets) so that the app user can navigate to see a profile's details.
...
Create a [development build](/develop/development-builds/introduction.md#how-would-you-like-to-build-your-development-build) and set the [`UIDesignRequiresCompatibility`](https://developer.apple.com/documentation/BundleResources/Information-Property-List/UIDesignRequiresCompatibility) property to `true` in [app config](/workflow/configuration.md):
...
The `expo-router/js-stack` entry point is the SDK 56 replacement for the `@react-navigation/stack` library. See the [SDK 55 to 56 migration guide](/router/migrate/sdk-55-to-56.md) for more information.
