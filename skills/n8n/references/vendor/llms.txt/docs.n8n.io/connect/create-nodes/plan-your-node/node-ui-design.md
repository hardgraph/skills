> For the complete documentation index, see [llms.txt](https://docs.n8n.io/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.n8n.io/connect/create-nodes/plan-your-node/node-ui-design.md).
...
All node's use n8n's [node UI elements](/connect/create-nodes/build-your-node/reference/node-ui-elements.md), so you don't need to consider style details such as colors, borders, and so on. However, it's still useful to go through a basic design process:
...
| ------------------------------------------------------------------------------------------------------------------- | --------------- | ------------------------------- |
| <p>If a node is a trigger node,<br>the displayed name should have 'Trigger' at the end,<br>with a space before.</p> | Shopify Trigger | ShopifyTrigger, Shopify trigger |
...
* Info boxes: yellow boxes that appear between fields. Refer to [UI elements | Notice](/connect/create-nodes/build-your-node/reference/node-ui-elements.md#notice) for more information.
...
* Node hints: provide help in the input panel, output panel, or node details view. Refer to [UI elements | Hints](/connect/create-nodes/build-your-node/reference/node-ui-elements.md#hints) for more information.
* Tooltips: callouts that appear when the user hovers over the tooltip icon !["Screenshot of the tooltip icon. The icon is a ? in a grey circle"](/files/X28ErwdzpUMyxaMX2dyl). Use tooltips for extra information that the user might need.
...
Refer to [Node Error Handling](/connect/create-nodes/build-your-node/reference/error-handling.md) for more information.
...
* Wherever possible, provide two ways to specify a record:
  * By choosing from a pre-populated list. You can generate this list using the `loadOptions` parameter. Refer to [Base files](/connect/create-nodes/build-your-node/reference/base-files.md) for more information.
  * By entering an ID.
* Name the field `<Record name> name or ID`. For example, **Workspace Name or ID**. Add a tooltip saying "Choose a name from the list, or specify an ID using an expression." Link to n8n's [Expressions](/build/work-with-data/expressions-versus-data-nodes.md) documentation.
