...
[Apify](https://apify.com) is a platform built to serve large-scale and high-performance web scraping and automation needs. It provides easy access to compute instances (*Actors*), convenient [request](https://docs.apify.com/sdk/js/sdk/js/docs/concepts/request-storage.md) and [result](https://docs.apify.com/sdk/js/sdk/js/docs/concepts/result-storage.md) storages, [proxies](https://docs.apify.com/sdk/js/sdk/js/docs/concepts/proxy-management.md), [scheduling](https://docs.apify.com/scheduler), [webhooks](https://docs.apify.com/webhooks) and [more](https://docs.apify.com/), accessible through a [web interface](https://console.apify.com) or an [API](https://docs.apify.com/api).
...
You can also develop your Actor in an online code editor directly on the platform (you'll need an Apify account). Let's go to the [Actors](https://console.apify.com/actors) page in the app, click **Develop new**, choose a template, go to the **Source** tab and start writing the code or paste one of the examples from the Examples section in the sidebar.

## Graceful shutdown[](#graceful-shutdown)

When running on the Apify platform, your Actor may receive signals to shut down gracefully. This happens when:
...
By default, the SDK automatically handles these events:
...
// Register cleanup handler — runs on both exit and reboot (or just use `Actor.useState`)

Actor.on('persistState', async () => {

    console.log('Saving state...');

    await Actor.setValue('STATE', myState);
...
You can add a delay before the shutdown action is triggered using `gracefulShutdownDelayMillis`:

```
await Actor.init({

    gracefulShutdownDelayMillis: 5000, // Wait 5 seconds before exiting/rebooting

});
```

To disable automatic graceful shutdown handling, set `gracefulShutdown` to `false`:
