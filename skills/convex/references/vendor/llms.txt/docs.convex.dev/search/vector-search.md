...
Vector search is consistent and fully up-to-date. You can write a vector and immediately read it from a vector search. Unlike [full text search](/search/overview.md), however, vector search is only available in [Convex actions](/functions/actions.md).
