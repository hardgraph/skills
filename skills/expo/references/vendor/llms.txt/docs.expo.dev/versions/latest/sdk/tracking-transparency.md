...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/versions/latest/sdk/tracking-transparency/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/latest/sdk/tracking-transparency/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
To localize this iOS permission message, keep `userTrackingPermission` as the default value and add `NSUserTrackingUsageDescription` to the `ios` object in each [locale file](/guides/localization.md#translating-app-metadata). Expo writes the localized values to **InfoPlist.strings** during prebuild.
