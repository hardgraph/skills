...
A component's functions are isolated from the app's environment variables and cannot access just any value from the app's `process.env`. Only the [system environment variables](/production/environment-variables.md#system-environment-variables) (`CONVEX_CLOUD_URL` and `CONVEX_SITE_URL`) are available in every component function. For anything else, components can declare their own typed environment variables in `convex.config.ts`:
...
If a component declares required environment variables, the app that installs the component must provide values for these variables:
...
import myComponent from "../component/convex.config";



const app = defineApp();

app.use(myComponent, { env: { MODE: "test" } });

export default app;
```

If the app also declares environment variables, it can pass them **by reference** so the component always sees the current value:
...
app.use(myComponent, { env: { MODE: app.env.ENVIRONMENT } });

export default app;
```

Inside the component, access the typed variables via the `env` export from `_generated/server`:

component/translate.ts
...
export const translate = action({
  ...
  handler: async (ctx, args) => {
    ...
    // env.API_KEY is typed as `string | undefined`

    // env.MODE is typed as `"test" | "live"`
    ...
    const response = await fetch("https://api.example.com/translate", {
      ...
      body: JSON.stringify({ text: args.text, mode: env.MODE }),
...
Environment variables declared by a component are also available via `process.env` but without the type safety of the `env` object described above. All attributes accessed on `process.env` are typed as `string | undefined` so typos and incorrect values will slip through to runtime. Having the values on `process.env` can come in handy though if you are using a third-party library that expects a certain variable to be present there.

If the component does not declare environment variables, apps can still pass values as function arguments as an alternative approach:
...
A component's declared environment variables are bound to the installed component instance and only exist at runtime. Read them inside function handlers rather than at module scope: module-scope code also runs during deployment analysis, where these values are not yet defined.

### HTTP Actions[​](#http-actions "Direct link to HTTP Actions")

Components can define their own `http.ts` file with HTTP routes, just like the main app. The app that installs the component chooses where these routes are accessible by specifying an `httpPrefix` (see [Using Components: HTTP Routes](/components/using.md#http-routes)).

Here’s what a component’s `http.ts` looks like:

convex/components/myComponent/http.ts
...
If your component has child components that define HTTP routes, you can mount them with their own `httpPrefix`:
...
import childComponent from "../childComponent/convex.config.js";



const component = defineComponent("myComponent");

component.use(childComponent, { httpPrefix: "/child/" });

export default component;
```

Within all of a component’s functions, `process.env.CONVEX_SITE_URL` reflects the full URL where the component is mounted, including the `httpPrefix`. This allows the component to construct correct URLs for callbacks, redirects, or self-referencing links without needing to know its mount path at build time.

**Limitation:** Component HTTP actions do not have access to `ctx.auth` or the app’s environment variables. If an HTTP handler needs to authenticate users or access data from the application, define the HTTP action handler in the component’s client code and mount it in the app’s `convex/http.ts` instead. There’s an example of this pattern in the [Twilio component](https://github.com/get-convex/twilio/blob/0bdf7fd4ee7dd46d442be3693280564eea597b68/src/client/index.ts#L71).
...
If your configuration can be expressed as strings or unions of string literals, consider using the environment variables support covered above.

For other types of configuration values, a common pattern is to have a "globals" table with a single document that contains the configuration. You can then define functions to update this document from the CLI or from the app. To read the values, you can query them with `ctx.db.query("globals").first();`.
