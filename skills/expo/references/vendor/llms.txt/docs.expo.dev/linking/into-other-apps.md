...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/linking/into-other-apps/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/linking/into-other-apps/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Into other apps](https://docs.expo.dev/linking/into-other-apps.md) (this page)
...
-   Using the [`expo-linking`](/versions/latest/sdk/linking.md) API
-   Using Expo Router's [`Link` component](/develop/app-navigation.md)

## Using expo-linking API

The [`expo-linking`](/versions/latest/sdk/linking.md) API provides a universal abstraction over native linking APIs (such as `window.history` on the web) and offers utilities for your app to interact with other installed apps.

The example below opens at the [common URL scheme](/linking/into-other-apps.md#common-url-schemes) in the default browser of the operating system using [`Linking.openURL`](/versions/latest/sdk/linking.md#linkingopenurlurl):
...
If your project uses [Expo Router](/router/introduction.md), use the `Link` component to open a URL. It wraps a `<Text>` component on native platforms and an `<a>` element on the web. It also uses the `expo-linking` API to handle URL schemes.

The example below opens a [common URL scheme](/linking/into-other-apps.md#common-url-schemes) (HTTPS) in the default browser of the operating system:
...
For Android 11 (API level 30) and above, you must specify the intents your app will handle in the **AndroidManifest.xml** file. You can accomplish this by [creating a config plugin](/config-plugins/plugins.md#creating-a-config-plugin).
...
After creating the config plugin, [import the custom config plugin](/config-plugins/plugins.md#call-the-config-plugin-from-your-dynamic-app-config) under the `plugins` property:
...
> **Tip**: On Android, you can use `expo-intent-launcher` to open a **specific settings screen** on the device. See [`expo-intent-launcher` API reference](/versions/latest/sdk/intent-launcher.md#enums) to view the list of available intents.

## Custom URL schemes

> If you know the custom scheme for the app you want to open, you can link to it using any of the methods: [Using `expo-linking` API](/linking/into-other-apps.md#using-expo-linking-api) or [Using `Link` from Expo Router](/linking/into-other-apps.md#using-expo-routers-link-component).
...
On iOS, using [`Linking.canOpenURL`](/versions/latest/sdk/linking.md#linkingcanopenurlurl) to query other apps's linking schemes requires additional configuration in **InfoPlist**. You can use the [`ios.infoPlist`](/versions/latest/config/app.md#infoplist) property in your app config to specify a list of schemes your app is allowed to query. For example:
...
> **Tip**: To test the configuration described above from an iOS device, [use a development build](/develop/development-builds/introduction.md). It cannot be tested with Expo Go.

## Create URLs

You can use [`Linking.createURL`](/versions/latest/sdk/linking.md#linkingcreateurlpath-namedparameters) to create a URL that can be used to open or redirect back to your app. This method resolves to the following:

-   **Production and development builds**: `myapp://`, where `myapp` is the [custom scheme](/linking/into-your-app.md#add-a-custom-scheme-in-app-config) defined in the app config
...
For apps that require a stable URL (for example, auth provider redirects), use a development build with a custom scheme instead of using Expo Go. For more details on how to create and test a custom scheme, see [Linking into your app](/linking/into-your-app.md).

## In-app browsers

The `expo-linking` API allows you to open a URL using the operating system's default web browser app. You can use the [`expo-web-browser`](/versions/latest/sdk/webbrowser.md) library to open URLs in an in-app browser. For example, an in-app browser is useful for secure [authentication](/guides/authentication.md).
...
To provide additional link functionality on the web, such as right-click to copy or hover to preview, you can use a `Link` component from the [`expo-router`](/router/introduction.md) library.
