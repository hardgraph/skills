...
url: https://docs.apify.com/integrations/slack.md
parents:
...
previous: [Pipedream](https://docs.apify.com/integrations/pipedream.md)
next: [Telegram](https://docs.apify.com/integrations/telegram.md)
---
...
![Apify Integrations tab listing available integration options with Configure buttons next to each](/assets/images/integrations-tab-407d3c50c55b932a1a4f3e055dcf2b5f.png)
...
![Slack integration setup form with event type selection, workspace, channel, and Message template fields](/assets/images/slack-integration-setup-0b413d14c705608f5d6a73e0ee5b5e05.png)
...
![Slack message field showing the /apify call command with an Actor or task ID](/assets/images/slack-apify-message-6c772c8d007770c873bfdc0f4201e80e.png)
...
The Slack integration uses Handlebars, so all built-in helpers (`{{#if}}`, `{{#each}}`, `{{#unless}}`, and so on) work in your template. [Webhook actions](https://docs.apify.com/integrations/webhooks/actions.md) use a separate JSON payload template engine; don't mix the two.
...
| `eventType` | string | Type of the trigger event. See [Webhook events](https://docs.apify.com/integrations/webhooks/events.md) for the full list. |
...
For `ACTOR.RUN.*` events, `resource` is the run document - the same object returned by the [Get run](https://docs.apify.com/api/v2/actor-run-get.md) API endpoint. Frequently used fields include:
...
| `resource.status`                 | string         | Current status of the run. See Status values.                               |
...
For `ACTOR.BUILD.*` events, `resource` is the build document - the same object returned by the [Get build](https://docs.apify.com/api/v2/actor-build-get.md) API endpoint. Frequently used fields include:

| Path                     | Type           | Description                                                                   |
| ------------------------ | -------------- | ----------------------------------------------------------------------------- |
| `resource.id`            | string         | Build ID.                                                                     |
| `resource.actId`         | string         | ID of the Actor being built.                                                  |
| `resource.status`        | string         | Current status of the build. See Status values.                               |
