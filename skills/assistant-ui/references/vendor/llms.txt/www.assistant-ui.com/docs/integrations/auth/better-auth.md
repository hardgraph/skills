...
1. ### Confirm the auth handler is mounted
...
3. ### Scope thread queries by user
...
5. ### Reload threads after async auth
...
- [Custom thread persistence](/docs/integrations/persistence/custom-adapter) — The persistence story this page builds on; required for per-user threads outside AssistantCloud.
- [Auth.js (next-auth)](/docs/integrations/auth/next-auth) — The other common Next.js auth library. Different session shape; same wiring approach.
- [Threads (concepts)](/docs/runtimes/concepts/threads) — The full RemoteThreadListAdapter contract and reload-on-auth pattern.