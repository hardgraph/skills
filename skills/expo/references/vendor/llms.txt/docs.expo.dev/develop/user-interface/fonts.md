...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/develop/user-interface/fonts/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/develop/user-interface/fonts/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Fonts](https://docs.expo.dev/develop/user-interface/fonts.md) (this page)
...
-   Embed the font file with [`expo-font` config plugin](/versions/latest/sdk/font.md#configuration-in-app-config) (Android and iOS only).
-   Load the font file with [`useFonts`](/versions/latest/sdk/font.md#usefonts) hook at runtime (Android, iOS, and web).
...
> **Note:** Config plugins only run on native platforms (Android and iOS). For web, use the [`useFonts` hook](/develop/user-interface/fonts.md#with-usefonts-hook) instead.
...
-   Doesn't work with Expo Go since this method requires [creating a development build](/develop/development-builds/introduction.md#how-would-you-like-to-build-your-development-build).
...
Add the config plugin to your [app config](/versions/latest/config/app.md#plugins) file. The configuration must contain the path to the font file using [`fonts`, `android` or `ios`](/versions/latest/sdk/font.md#configurable-properties) properties which take an array of one or more font definitions. The path to each font file is relative to the project's root.
...
If you plan to refer to fonts using just the `fontFamily`, provide an array of font paths (see `FiraSans-MediumItalic.ttf` below) and follow our [recommendation for file naming](/develop/user-interface/fonts.md#how-to-determine-which-font-family-name-to-use).
...
After embedding the font with the config plugin, create a [new development build](/develop/development-builds/introduction.md#how-would-you-like-to-build-your-development-build) and install it on your device or Android Emulator or iOS Simulator.
...
-   If you provide fonts as an array of file paths (as described above), on Android, the file name (without the extension) becomes the font family name. On iOS, the font family name is read from the font file itself. We recommend naming the font file same as its [PostScript name](/develop/user-interface/fonts.md#what-is-postscript-name-of-a-font) so the font family name is consistent on both platforms.
...
The [`expo-splash-screen`](/versions/latest/sdk/splash-screen.md) library provides `SplashScreen` component that you can use to prevent rendering the app until the font is loaded and ready.
...
-   Embed the installed font with [`expo-font` config plugin](/versions/latest/sdk/font.md#configuration-in-app-config).
-   Load the installed font with [`useFonts`](/versions/latest/sdk/font.md#usefonts) hook at runtime asynchronously.

### With `expo-font` config plugin

> **Note:** Embedding a Google Font using `expo-font` config plugin has same benefits and limitations as embedding a custom font on your own. See [using a local font file with `expo-font` config plugin](/develop/user-interface/fonts.md#with-expo-font-config-plugin) for more information.
...
Add the config plugin to your [app config](/versions/latest/config/app.md#plugins) file. The configuration must contain the path to the font file using [`fonts`](/versions/latest/sdk/font.md#configurable-properties) property which takes an array of one or more font files. The path to the font file is defined from the font package inside the `node_modules` directory. For example, if you have a font package named `@expo-google-fonts/inter`, then the name of the file is **Inter_900Black.ttf**.
...
On Android, you can use the font file name. For example, `Inter_900Black`. On iOS, use the font and its weight name ([PostScript name](/develop/user-interface/fonts.md#what-is-postscript-name-of-a-font)). The example below demonstrates how to use [`Platform`](https://reactnative.dev/docs/platform-specific-code#platform-module) to select the correct font family name for each platform:
...
> **Note:** Loading a Google Font using `useFonts` hook has same benefits and limitations as embedding a custom font on your own. See [using a local font file with `useFonts` hook](/develop/user-interface/fonts.md#with-usefonts-hook) for more information.
...
[expo-font usage](/versions/latest/sdk/font.md#usage) — expo-font — See usage section in Expo Fonts API reference for a minimal example of using a custom font.

### Beyond OTF and TTF

If your font is in format other than OTF or TTF, you have to [customize the Metro bundler configuration to include it as an extra asset](/guides/customizing-metro.md#adding-more-file-extensions-to-assetexts) for it to work. In some cases, rendering a font format that a platform doesn't support may cause your app to crash.
