...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/modules/autolinking/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/modules/autolinking/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Autolinking](https://docs.expo.dev/modules/autolinking.md) (this page)
...
Expo Autolinking links both Expo modules and React Native modules. To use the React Native community CLI autolinking instead, see [opting out of Expo Autolinking for React Native modules](/modules/autolinking.md#opting-out-of-expo-autolinking-for-react-native-modules) section.
...
A path relative to the app's root directory that Expo Autolinking should search for local modules to autolink. This option defaults to `"./modules"`. Changing this option is only useful if you need to change the path [for local Expo modules](/modules/get-started.md).
...
A list of package names to opt out of prebuilt Expo modules. For complete reference, see [Prebuilt Expo Modules for Android](/guides/prebuilt-expo-modules.md#disabling-specific-modules-via-expo-autolinking).
...
This is an especially common problem with isolated dependencies or monorepos, and you should [check for and deduplicate native modules in your dependencies](/guides/monorepos.md#duplicate-native-packages-within-monorepos).

From **SDK 54**, you can set `experiments.autolinkingModuleResolution` to `true` in your [app config](/workflow/configuration.md) to apply autolinking to Expo CLI and Metro bundler automatically. This will force dependencies that Metro resolves to match the native modules that **autolinking** resolves.
...
All projects created with the `npx create-expo-app` command are already configured to use Expo Autolinking. If your project was created using a different tool, see [Installing Expo modules](/bare/installing-expo-modules.md) to make sure your project includes all necessary changes.

### What do I need to have in my module to make it autolinkable?

The module resolution algorithm searches only for packages that contain the [Expo module config](/modules/module-config.md) file (**expo-module.config.json**) at the root directory, next to the **package.json** file. It's also necessary to include supported platforms in the `platforms` array — if the platform for which the autolinking algorithm is run is not present in this array, it's just skipped in the search results.
