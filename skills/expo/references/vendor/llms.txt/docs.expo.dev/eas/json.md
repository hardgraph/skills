...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/eas/json/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/eas/json/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [Configuration with eas.json](https://docs.expo.dev/eas/json.md) (this page)
...
**eas.json** is the configuration file for EAS CLI and services. You can find the complete reference of all available schema properties for [EAS Build](/build/introduction.md) and [EAS Submit](/deploy/submit-to-app-stores.md) on this page.

> To learn more about how a project using EAS services is configured with **eas.json**, see [Configure EAS Build with eas.json](/build/eas-json.md) and [Configure EAS Submit with eas.json](/submit/eas-json.md).
...
| --- | --- |
| `withoutCredentials` | **(boolean)** - When set to `true`, EAS CLI won't require you to configure credentials when building the app. This comes in handy when using EAS Build [custom builds](/custom-builds/get-started.md). Defaults to `false`. |
...
| `prebuildCommand` | **(string)** - Optional override of the [prebuild](/more/expo-cli.md#prebuild) command used by EAS. For example, you can specify `prebuild --template example-template` to use a custom template. **Note**: `--platform` and `--non-interactive` will be added automatically by the build engine, so you do not need to specify them manually. |
...
| `bun` | **(string)** - Version of Bun used for build. You can also use a specific version. Learn [how to configure the exact version in eas.json](/guides/using-bun.md#customize-bun-version-on-eas). |
| `expoCli` | **(string)** - **Deprecated**: Version of [`expo-cli`](https://www.npmjs.com/package/expo-cli) used to [prebuild](/more/expo-cli.md#prebuild) your app. It only affects managed projects on Expo SDK 45 and lower. For newer SDKs, EAS Build will use the versioned [Expo CLI](/more/expo-cli.md). It is included with `expo` library. You can opt out of using the versioned Expo CLI by setting the `EXPO_USE_LOCAL_CLI=0` environment variable in the build profile. |
| `env` | **(object)** - [Environment variables](/guides/environment-variables.md) that should be set during the build process. It should only be used for values that you would commit to your git repository and not for passwords or [secrets](/eas/environment-variables.md#visibility-settings-for-environment-variables). |
...
| `config` | **(string)** - Custom workflow file name that will be used to run this build. You can also specify this property on platform level for platform-specific workflows. [Learn more](/custom-builds/get-started.md). Example: `"config": "production.yml"` will use workflow from `.eas/build/production.yml`. |
| `environment` | **(enum: development, preview, production)** - The environment used to apply environment variables for the build process. [Learn more](/eas/environment-variables.md). |
...
. Based on the value of [`cli.appVersionSource` in **eas.json**](/build-reference/app-versions.md), the values will be updated locally in your project or on EAS servers. |
...
| `config` | **(string)** - Custom workflow file name that will be used to run this Android build. You can also specify this property on profile level for platform-agnostic workflows. [Learn more](/custom-builds/get-started.md). Example: `"config": "production-android.yml"` will use workflow from `.eas/build/production-android.yml`. |
...
-   For a [bare React Native](/bare/overview.md) project, defaults to the value specified in the scheme.
...
| `config` | **(string)** - Custom workflow file name that will be used to run this iOS build. You can also specify this property on profile level for platform-agnostic workflows. [Learn more](/custom-builds/get-started.md). Example: `"config": "production-ios.yml"` will use workflow from `.eas/build/production-ios.yml`. |
...
| `appName` | **(string)** - The name of your app as it will appear on the App Store. Defaults to `expo.name` from the [app config](/workflow/configuration.md). |
...
| `metadataPath` | **(string)** - The path to your [store configuration file](/eas/metadata.md). |
