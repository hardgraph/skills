...
Preferred command:
npx --yes submit-expo-feedback@latest --category docs --subject "/guides/dom-components/" "<actionable feedback>"

Direct HTTP fallback:
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/guides/dom-components/","feedback":"🤖 Agent feedback for docs: <specific, actionable description> (<model>, <harness>)"}'
...
- [DOM components](https://docs.expo.dev/guides/dom-components.md) (this page)
...
##### Expo SDK 55 and earlier: react-native-webview
...
#### How to opt out of @expo/dom-webview and use react-native-webview instead?
...
If you use an AI agent, install [Expo Skills](/skills.md) to teach it when to reach for DOM components and how to migrate web code to native:

[expo-dom](https://github.com/expo/skills/blob/main/plugins/expo/skills/expo-dom/SKILL.md) — Use Expo DOM components to run web code in a webview on native and as-is on web.

[expo-web-to-native](https://github.com/expo/skills/blob/main/plugins/expo/skills/expo-web-to-native/SKILL.md) — Migrate an existing web React app to a native iOS/Android app with Expo.
...
-   DOM component bundles can be introspected in [Expo Atlas](/guides/analyzing-bundles.md#analyzing-bundle-size-with-expo-atlas) for debugging.
...
> Public assets are not supported in EAS Update. Use `require()` to load local assets instead.
...
-   **Debug builds**: When using development servers (which default to the `http://` protocol), you can use [tunneling](/more/expo-cli.md#tunneling) to serve DOM components over HTTPS.
